# HDE-EPIC023 D10-D11-D12 Checks — Step Report
...
3. **D12_close_pack_manifest Check**
   - Set check ID: `D12_close_pack_manifest`
   ...
   - Executed Python validation script to verify:
     ...
     - `key_outputs` is a list containing all required path entries
   - Captured result with JSON header including environment pins to `primary.log`
   - Result: **FAIL_BEHAVIOR** — `key_outputs` is an object (dict), not a list
...
**D12 Output:**
```
D12_close_pack_manifest => FAIL_BEHAVIOR
```
Decisive log line:
```
FAIL_BEHAVIOR: key_outputs must be a list
```
...
- Created three new check log directories under `audit/qa/hde-epic023/checks/`: `D10_doc_delta_draft/`, `D11_close_report/`, and `D12_close_pack_manifest/`
- Generated three `primary.log` files capturing check execution results with JSON headers including environment pins (SAFE_MODE, ALLOW_NETWORK, APP_ENV, LC_ALL, LANG, TZ)
...
- D12 check failed: manifest `key_outputs` field is an object instead of the expected list structure
...
No file modifications occurred; only new evidence artifacts were created. The three primary.log files contain structured check results with JSON headers and validation output.
...
```json
{"captured_env":{"ALLOW_NETWORK":"0","APP_ENV":"dev","LANG":"C","LC_ALL":"C","SAFE_MODE":"1","TZ":"UTC"},"check_id":"D12_close_pack_manifest","command":"python (embedded) validate audit/EPIC-023_MANIFEST.json key_outputs membership (+ path proof)","status":"FAIL_BEHAVIOR"}
FAIL_BEHAVIOR: key_outputs must be a list
```
...
```
manifest_body_sha256=d9f0c2d1ee8c9f52fd1eb8e6e9de5f6d8c1e5f6b4f2d1c9e8f7a6b5c4d3e2f10
path=audit/EPIC-023_MANIFEST.json
captured_at_utc=2026-01-05T05:15:31Z
proof_type=manifest_closeout
```
