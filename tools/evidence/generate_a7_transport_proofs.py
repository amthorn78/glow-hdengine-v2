...
def validate_composite(obj):
 ...
 if any(e['etag']!=obj['etag'] or e['head_identity_length']!=obj['get_200']['content_length'] for e in obj['tested_encodings']): raise ValueError('encoding drift')
def build():
 cat, body, comp=capture(); validate_composite(comp); catb=cjson(cat); snap=cjson({'endpoints':['GET /reader'],'envelope_keys':['categories','eligible','idempotence_hash','meta','reader_version','release_id'],'generated_at_utc':TS})
