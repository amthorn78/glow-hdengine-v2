...
| EVIDENCE_INDEX_HASH_OK | PF12 — Schemas & Artifacts §Evidence Hash Discipline | docs/evidence/INDEX.sha256; artifacts/evidence_index.jsonl | python tools/evidence/update_evidence_index.py | acceptance_map_viability.log | Implemented | Bound to the Human Index sentinel and governed Machine Mirror self-record/path-proof integrity. |
| ENV_RAILS_POLICY_OK | PF10 — HDE Build Notes §Closed Rails | audit/gates/determinism/env_pins.log | ci/checks/check_env_pins.sh | acceptance_map_viability.log | Implemented | Determinism env pins evidence remains present for closed-rails posture. |
