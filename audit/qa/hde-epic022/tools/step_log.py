...
def main():
    ...
    # Convenience copy: keep a mirror copy inside step_logs/
    step_logs_manifest_copy = Path(str(log_path.parent / "qa_step_logs_manifest.json"))
    step_logs_manifest_copy.write_text(manifest_path.read_text(encoding="utf-8"), encoding="utf-8")

    sys.exit(0 if status == STATUS_PASS else 1)
