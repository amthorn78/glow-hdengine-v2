...
def recompute(
    *,
    ...
    if not check:
        ...
        _write_sha256_sidecar(freeze_path, manifest_digest)
        _write_text(release_id_path, expected_release_id + "\n")
        _write_sha256_sidecar(release_id_path, hashlib.sha256((expected_release_id + "\n").encode("utf-8")).hexdigest())
        if manifest_obj is not None:
            audit = _audit_files(manifest_obj)
            _write_text(checksums_path, "\n".join(audit) + "\n")
            snapshot_payload = {
                "manifest": {
                    "root": manifest_obj.root,
                    "version": manifest_obj.version,
                    "built_at_utc": manifest_obj.built_at_utc,
                    "files": [
                        {"path": entry.path, "sha256": entry.sha256, "size": entry.size} for entry in manifest_obj.files
                    ],
                },
    ...
    log_lines = [
        ...
        f"release_id_txt={release_id_value or ''}",
        f"expected_release_id={expected_release_id}",
        f"freeze_pack_sha256={freeze_digest or ''}",
    ]
    log_lines.append(f"match={str(not problems).lower()}")
    log_lines.append(f"problems_count={len(problems)}")
