...
**urrent PF09 recorded status is not a pre-drain closure gate.** PF09 remains the checklist mapping and later-drain record for this phase. Current PF09 recorded status text may be cited as canon-as-recorded, but it is not by itself a PR acceptability gate, OPS acceptability gate, QA-entry gate, or epic-close gate.

Closure and acceptability decisions for mapped Conjunction work MUST be based on whether the exact mapped PF09.x task or subtask is complete in substance from:

* approved implementation state  
* approved OPS state, where applicable  
* governed evidence  
* truthful review and approval artifacts  
* PF10 live truth, where PF10 explicitly covers the mapped work

If a slice maps to an exact PF09.x subtask, that subtask is the controlling unit. Parent-task language is not enough when a subtask exists. If a slice claims to close more than one PF09.x subtask, each claimed subtask must be truthfully complete in substance before acceptable-status language is allowed.

Green tests, bounded diff scope, passing evidence refresh, successful OPS execution, or review-clean artifact posture are necessary but not sufficient by themselves. They do not permit acceptable-status language if the mapped work still remains open in substance.

**Review-language discipline before drain.**

Before mapped work is complete in substance, allowed review-language labels are:
...
Acceptable-status language such as `acceptable`, `accepted`, `satisfied`, `complete-for-close`, and `supportable for later drain to Done` may be used only when the mapped work is complete in substance, governed evidence proves that posture, and PF10 records that live truth where PF10 speaks.

Do not block closure or acceptability solely because PF09 currently says `Partial`, `Not done`, or another pre-drain state. Do not write “not acceptable because PF09 still says Partial” or “PF09 must already be edited to Done before acceptance language may be used.”

**Bounded intermediate task review scope.** When an approved PR or OPS task is explicitly validation-only, evidence-only, sequencing-only, classification-only, or otherwise non-closure, that task must be reviewed on whether it truthfully completes its own approved job.

For those bounded non-closure steps, PF09 row closure is not a review gate unless the approved task explicitly claims one or more of the following:

* it brings a mapped PF09 row to Done  
* it supports a Done recommendation now  
* it performs final closure, final binding, final acceptance promotion, or another explicitly closure-claiming action

When a bounded step does not claim closure, the review must instead confirm that it stays within approved scope, does not overclaim closure, preserves any still-open PF09 row truthfully as open or intermediate, and does not imply that later closure work is already complete.

Do not reject or fail an approved bounded step solely because later work remains for the mapped PF09 row.

**Later-drain approval-artifact statement is mandatory.**

Any approval artifact intended to support later PF-canon drain for a PF09.4 row MUST state all of the following explicitly:

* `Affected PF canon home(s)`  
* `Exact affected locator(s)`  
* `Current canon posture`  
* `Supported later-drain action` with exactly one of:  
  * `change to Done`  
  * `change to Partial`  
  * `change to Not done`  
...
* `Drain readiness classification` with exactly one of:  
  * `Supportable from repo evidence`  
  * `Not yet supportable from repo evidence`  
  * `Already drained into PF-canon`  
* `Evidence basis`  
* `Epic-close expectation` with exactly one of:  
  * `at epic close`  
  * `after an additional PR or OPS slice`  
  * `after a separate canon-only drain step`

Approval artifacts MUST NOT stop at vague wording such as `accepted`, `complete`, `merge-ready`, `approved`, or `no further remediation needed` when the practical intent is to support a later PF-canon update. They must say what later drain they support.

**Documentation drainage is never an execution gate.** PF10 drainage and any later documentation drainage into PF04, PF06, PF07, PF09.x, PF12, PF19, PF27, or other canon or guide homes are never prerequisites, required deliverables, required checks, acceptance conditions, or blockers by themselves.

If a canon or checklist delta is known but not yet drained, PF10 remains the temporary live-truth home until drainage occurs. Work, review verdicts, closeout recommendations, and readiness judgments must follow that live truth and governed evidence rather than waiting on drain.

Allowed blockers remain limited to real truth and proof failures, including:

* incomplete required QA steps  
* missing required deliverables  
* untrusted or non-governed evidence  
* unresolved `FAIL_BEHAVIOR`, `FAIL_TOOLING`, or `TOOLING_BLOCKED` conditions that affect acceptance  
* missing required close-gate QA artifacts

Documentation mismatches must be recorded first and drained later. Plans, reviews, QA artifacts, OPS tasks, closeout artifacts, and acceptance artifacts may name later drain targets, doc-delta candidates, or supportable-from-repo-evidence posture, but they must not require that drainage already be complete for the current artifact to stand.

No plan may mandate PF document edits as required execution outputs for PR work, OPS work, QA execution, step completion, or closeout readiness.

Wording that says or implies `drain required before close`, `cannot pass until PF10 is drained`, `not ready because canon is not yet drained`, `PF update required before acceptance`, or any equivalent drain-required posture is non-conforming for this phase doc.

This rule does not weaken truthfulness, evidence, governed-artifact, close-gate QA, acceptance-token, or canonical-ownership requirements. It changes only blocker posture: documentation drainage itself is never the thing that makes otherwise-proven Conjunction work invalid.
