...
##### **Ordering evidence drift: `abba_identity.bytes` proof/mirror mismatch**

**Issue:** During EPIC022 close-pack and evidence-binding review, the governed ordering artifact `artifacts/engine/order/abba_identity.bytes` was found to have mismatched metadata between the on-disk bytes and its sibling path-proof and/or Machine Mirror record.

**Why it matters:** This breaks path-proof validation and can cause validators to fail or, worse, certify incorrect evidence.
