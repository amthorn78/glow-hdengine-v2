...
**Last Update Gate:** redlines-PF12-Canon-HDE-Schemas-and-Artifacts-v2.9.3-from-PF10-HDE-Build-Notes-v12.9  
...
### **PF09 and PF30 work accountability**
...
* A mapped phased HDE Build Checklist task or subtask for Epic/PF09 work.  
* A phased HDE Build Checklist gap for Epic/PF09 work.  
* A registered CRD in the PF30 CRD Register for CRD-originated work outside PF09.  
* Work outside both governed work registers.  
* Documentation or status drainage only.

When both a relevant PF09 parent and subtask exist, use the subtask. PF12 does not create HDE Build Checklist rows, PF30 CRD records, or status decisions, and it does not transform an evidence note into an implementation obligation.

Epic-originated phased product work maps to PF09. CRD-originated work maps to PF30. Work moves or maps between those populations only through an explicit authorized decision.
...
Change Process Guide owns PR workflow. PF12 defines which catalogs, manifests, ledgers, proofs, and companions must remain synchronized, not the mechanics for managing the PR.
...
Add the applicable change record and Doc-Delta hook under §9. Change Process Guide owns the PR procedure.
...
Change-container compatibility (normative).

This generic close-pack contract supports both Epic/PF09 and CRD/PF30 work.

* `EPIC-\#\#\#` and `<epic-id>` remain valid for historical and active epics.  
* For a CRD, the stable CRD ID recorded in PF30 replaces `EPIC-\#\#\#` as the filename prefix in the baseline suffix patterns below and replaces `<epic-id>` in supporting paths using its lower-case ASCII, hyphenated directory form.  
* In CRD close-pack use, generic references below to an epic, epic close, or PF09 pointer or register mean the corresponding CRD, CRD close, or PF30 pointer or register. Named HDE-EPIC-specific families remain epic-specific.  
* The same close report, manifest, `key_outputs`, Doc-Delta, drain-target, QA-manifest, Human Evidence Index, Machine Evidence Mirror, checksum, and path-proof contracts apply to both lanes.  
* Existing historical and active epic artifacts remain valid and MUST NOT be renamed or rewritten merely to add CRD compatibility.

Before the first affected CRD close-pack enters QA or closeout, every schema, generator, or validator that accepts only `EPIC-\#\#\#` or `<epic-id>` MUST be identified and updated through the authorized change lane.
...
This section defines the current catalog homes and closed domains consumed by the engine. Canonical bytes remain single-homed at their governed repository paths and are not duplicated here. A path not named in this section is not part of the current PF12 catalog surface. A new catalog home, standalone schema, enriched field, or closed-domain change requires a Doc-Delta under §9.

## **2.1 Topology catalogs**

### **Gates and centers**

**Current repository surface.** The Gate Catalog is `catalog/gates_v1.json`; its declarative schema home is `schemas/gates_v1.schema.json`. The catalog contains one top-level `gates` array with 64 rows. Each row contains exactly an integer `gate` and one machine `center` ID.

**Required contract.** Gate IDs are the unique integers `1..64`. The closed center-ID set is:
...
Centers are derived from the Gate Catalog. No separate Centers catalog or schema belongs to the current implemented catalog surface. For machine data, `g` is the canonical ID for the G Center and `ego` is the canonical ID for the Heart/Ego Center. Descriptive or legacy forms such as `g_center` and `ego_heart` are not canonical catalog IDs. An explicitly governed ingestion alias MAY map to a canonical ID; canonical outputs contain only canonical IDs.

### **Channels**

**Current repository surface.** The Channel Catalog is `catalog/channels_v1.json`; its declarative schema home is `schemas/channels_v1.schema.json`. The catalog contains one top-level `channels` array with 36 primary rows. Each row contains exactly `id`, `gates`, `centers`, `circuit_primary`, `substream`, `primary_domain`, `domains`, and `flags`.
...
`engine/config/registry_loader.py` is the executable topology-validation home. It MUST fail closed on missing files, malformed shapes, duplicate IDs, unknown Gate or center references, malformed or noncanonical Channel IDs, Channel/Gate identity mismatches, endpoint-center set mismatches, and aliases outside the explicit input-only policy. `artifacts/registry/registry_report.json` is the generated projection governed by §8.5; it is not a second catalog.

## **2.2 Authorities and Profiles \[Future-Promotion\]**

**Current implementation boundary.** The current catalog loader, root manifest, topology schemas, and registry projection do not define or consume separate Authorities or Profiles catalogs. These domains are excluded from current catalog-validation and Freeze-Pack scope. This exclusion does not retire the Human Design concepts.

**Future-Promotion requirement.** Introducing either catalog requires, in the same governed change, an exact catalog path, a schema or executable validation contract, consumer wiring, Freeze-Pack Manifest treatment, tests, evidence, and a Doc-Delta.

## **2.3 Magic-10 closed ID set**

The Magic-10 closed ID set is:

`alignment`, `balance`, `comfort`, `communication`, `consistency`, `creativity`, `drive`, `expansion`, `harmony`, `heat`.

No consumer, viewer-preferences object, configuration artifact, seed map, narrative key, or evidence artifact may introduce another Magic-10 ID.

## **2.4 Viewer preferences**

**Required contract.** A viewer-preferences object contains exactly `top_category` and `weights`.

* `top_category` MUST be one ID from §2.3.  
* `weights` MUST contain exactly all ten §2.3 IDs as keys.  
* Every weight MUST be an integer in `0..100`; booleans and non-integer numbers are invalid.  
* Unknown, missing, or duplicate category identities fail closed.  
* A zero weight is preserved for the sampler or ranker exclusion decision; normalization MUST NOT replace it with a positive value.

JSON object-key serialization remains governed by §4. The semantic key set does not create a second category order.

## **2.5 Magic-10 caps and future Presets**

**Current repository surface.** The caps catalog is `catalog/magic10_caps.json`. It contains exactly the ten §2.3 IDs. Each row contains a non-empty `inputs` array and `bounds` whose current integer values are `min:0` and `max:100`.

**Required contract.** Unknown or missing category rows fail closed. Caps do not redefine the Magic-10 ID set, frozen order, or score thresholds.

**Future-Promotion.** No governed Presets catalog belongs to the current implemented catalog surface. A future Presets catalog MUST reference the §2.3 IDs and §2.6 order instead of embedding a second Magic-10 domain, order, cap, or threshold table. Promotion requires an exact catalog path, schema or executable validator, consumer wiring, Freeze-Pack Manifest treatment, evidence, tests, and a Doc-Delta.

## **2.6 Frozen Magic-10 order and bands**

The single home for the frozen Magic-10 emission order is `catalog/magic10.json`. The exact order is:
...
Consumers MUST use this order and MUST NOT define an alternate canonical order.

The band labels, in ascending order, are `Cool`, `Open`, `Warm`, and `Glow`. Numeric threshold arithmetic remains owned by HDE-Math-Spec. The current governed threshold input is `math/thresholds.json`, which carries clamp bounds, inclusive edges, rounding mode, and version. Caps live in `catalog/magic10_caps.json`; neither caps nor threshold edges live in `catalog/magic10.json`.

## **2.7 Magic-10 seed metadata**

**Current repository surface.** The seed-metadata catalog is `catalog/magic10_seeds.json`. It is a partial map with current rows for `harmony` and `heat`. Each current row contains exactly `template_id`, `seed_version`, `updated_at_utc`, and `checksum_sha256`. No `admin_only` field belongs to the implemented shape.

**Required contract.** Catalog keys MUST be a subset of §2.3, and unknown category keys fail closed. `checksum_sha256` MUST be lowercase 64-hex and identifies the referenced seed content; it is not a digest of the metadata row itself.

Full-ten coverage, a governed seed-body catalog, or a new checksum projection is a Future-Promotion change requiring a Doc-Delta. None may be inferred from this metadata map.

Any byte change to `catalog/magic10_seeds.json` changes each manifest or generated configuration artifact that binds those bytes and requires the corresponding release-identity and evidence refresh under §§5–6 and §8.

## **2.8 Narratives pack**

**Current repository surface.** The narratives pack manifest is `catalog/narratives/manifest.json`. It identifies these four JSON members:
...
Each member and the pack manifest have sibling `.sha256` sidecars in the checked-in repository. Sidecar presence alone does not establish hash, size, canonical-byte, or release-coupling conformance.

**Required integrity contract.** Manifest member paths MUST be unique and ASCII-sorted. Each member record MUST bind `path`, `sha256`, and `size_bytes` to the member's canonical bytes. The pack manifest and all pack members MUST be canonical JSON under §4. Their canonical form includes exactly one final LF.

`pack_sha` is the SHA-256 digest of the canonical bytes of `catalog/narratives/manifest.json`. Composer and governed preview or evidence surfaces echo `pack_sha` as required by HDE-Narratives-Guide.
...
## **3.1 JSON Schema and catalog validation**

Every current catalog MUST pass its owning validation contract. A catalog with an owning JSON Schema MUST validate against that schema with no errors. Catalogs whose current contract is executable rather than schema-owned MUST pass the executable validator and every required companion check. The existence of a catalog does not imply an unlisted schema home.

### **Scope**

A catalog is a JSON artifact that enumerates a closed or structured domain consumed by the engine. An owning schema is the single declarative schema that defines the structure, types, and constraints for a catalog. The current declarative topology-schema homes are `schemas/gates_v1.schema.json` and `schemas/channels_v1.schema.json`; current executable validation is owned by the paths identified in §2.

### **Normative rules**

* Every property required by an owning schema or executable contract MUST be present.  
* Additional properties are invalid at a closed object level. A validator MUST reject unknown keys unless the owning contract explicitly permits them at that level.  
* Values MUST satisfy their owning types without coercion. In particular, booleans are invalid where an integer is required, and strings MUST NOT be converted to numeric values to make invalid input pass.  
* Every enum is closed. A value outside its owning enum is invalid.  
* An array declared as a set MUST contain no duplicate identity. If JSON Schema cannot express the identity or uniqueness rule, a companion check MUST enforce it.  
* An object-array declared as a set MUST have an owning identity projection for deduplication and ordering. Absence of that projection is a contract defect and MUST be covered by a companion check until corrected.  
* A simple string identifier such as `id` MUST be non-empty and MUST match `^[a-z0-9_]+$`, case-sensitively. A schema-defined composite identifier MUST match its own canonical projection. Any maximum length MUST be defined by the owning schema.  
* Numeric bounds and cardinalities MUST be enforced exactly as owned; validators MUST NOT widen them.  
* Seed metadata in `catalog/magic10_seeds.json` MUST use only keys from the Magic-10 domain. Every present row MUST contain exactly `template_id`, `seed_version`, `updated_at_utc`, and `checksum_sha256`. `checksum_sha256` MUST be lowercase 64-hex and identifies referenced seed content; it MUST NOT be recomputed over the metadata row itself unless a future governed seed-body projection defines that operation.

Schema and executable validation govern data shape, values, identity, and cross-reference integrity. Canonical bytes, key ordering, set ordering, encoding, and the final LF remain governed by §4.

### **Schema hygiene**

* Every owning JSON Schema MUST include both `$schema` and `$id`.  
* The required draft is JSON Schema 2020-12. `$schema` MUST be `https://json-schema.org/draft/2020-12/schema`.  
* `$id` MUST be a stable repository-path identity for its owning schema. Current schema examples are `schemas/gates_v1.schema.json` and `schemas/channels_v1.schema.json`. Reference-only UMS drafts are not owning schemas for the current catalogs.  
* Closed object levels SHOULD set `additionalProperties` to `false`; an open level MUST be intentional and explicit.  
...
* Catalog validation MUST run locally and in CI. A failure is a hard stop.  
* A change to a catalog, schema, executable validator, or referenced closed domain MUST rerun all dependent checks in the same governed change.  
* Uniqueness, cross-reference, ordering, identity-projection, and arrays-as-sets rules outside JSON Schema capability MUST run as companion checks.  
* Deterministic validation and ordering checks MUST run under `LC_ALL=C` as defined in §4.3.

Acceptance hints: `CATALOG_SCHEMA_OK`, `CATALOG_NO_ADDITIONAL_PROPS_OK`, `CATALOG_ENUM_DOMAIN_CLOSED_OK`, `ARR_SET_NO_DUPLICATES_OK`, `ARR_SET_IDENTITY_DECLARED_OK`, `SCHEMA_HYGIENE_OK`, and `SCHEMA_ID_STABLE_OK`.

## **3.2 Graph coherence checks**

Topology integrity is evaluated across the Gate and Channel catalogs, using the closed center domain derived from Gate rows under §2.1. No separate Centers catalog belongs to the current implemented surface.

### **Scope and derived projections**

Inputs are `catalog/gates_v1.json` and `catalog/channels_v1.json`.

* `center_of_gate[g]` is the canonical center ID owned by Gate `g`.  
* `gates_of_channel[ch]` is the two-Gate set declared by Channel `ch`.  
* `centers_of_channel_derived[ch]` is the set obtained by projecting both members of `gates_of_channel[ch]` through `center_of_gate`.

All comparisons of declared and derived sets use the set rules in §4.2.

### **3.2.1 Channel degree and identity**

#### **Cardinality**

* Each Channel MUST reference exactly two Gate IDs.  
* The two Gate IDs MUST be distinct and MUST exist in the Gate Catalog.  
* The Channel Catalog MUST contain exactly the 36 primary Channel identities owned by §2.1.

#### **Identity and uniqueness**

A Channel identity is the two referenced Gate IDs in numeric ascending order, zero-padded and encoded as `NN-NN`. Both components MUST be in `01..64`. The exact lexical pattern is `^(?:0[1-9]|[1-5][0-9]|6[0-4])-(?:0[1-9]|[1-5][0-9]|6[0-4])$`; matching the pattern does not replace the min-first or Gate-pair equality checks.

The `id` components MUST equal the two values in `gates` after canonical ordering. No two rows may share a canonical Channel identity. Repeated identity with identical or different row bytes is invalid and fails closed.

#### **Arrays-as-sets coupling**

Any array that lists Channels as a set MUST deduplicate by canonical `NN-NN` identity and MUST be ASCII-sorted by that identity. Duplicate, conflicting, or out-of-order identities are validation errors.

Acceptance hints: `TOPOLOGY_CHANNEL_DEGREE_2_OK`, `ARR_SET_NO_CONFLICTS_OK`, `ARR_SET_ASCII_SORT_OK`, `CATALOG_ORIENTATION_CANON_OK`, and `TOPOLOGY_COHERENCE_OK`.

### **3.2.2 Gate-center and Channel-center consistency**

* Every Gate MUST map to exactly one member of the closed center domain in §2.1.  
* For each Channel, the validator MUST derive `centers_of_channel_derived[ch]` from the two referenced Gate rows.  
* The two derived centers MUST be distinct; a Channel connects Gates belonging to two different centers.  
* The Channel row's `centers` set MUST equal `centers_of_channel_derived[ch]`. Positional equality between `gates` and `centers` is not required.  
* A declared center value that is closed-domain-valid but does not match the Gate-derived set is invalid.

Acceptance hints: `TOPOLOGY_CENTER_CONSISTENCY_OK`, `TOPOLOGY_DOMAIN_CLOSED_OK`, and `XREF_MEMBERSHIP_OK`.

### **3.2.3 No orphans**

* Every center ID referenced by a Gate MUST belong to the closed center domain in §2.1.  
* Every Gate ID referenced by a Channel MUST exist in the Gate Catalog.  
* Every explicit Channel center MUST be derivable from one of its two Gate rows.  
* There MUST be no dangling topology reference. A missing target is a hard validation failure.

Acceptance hint: `TOPOLOGY_NO_ORPHANS_OK`.

### **3.2.4 Declared degree vectors**

A degree vector is optional until an owning catalog or governed proof artifact declares it. When declared, the validator MUST compute `gate_degree[g]` as the number of distinct Channels that reference Gate `g` and compare the observed vector with the declared expectation.

* Every declared Gate key MUST exist in the Gate Catalog.  
* Every degree MUST be a non-negative integer.  
* Every declared value MUST equal the observed value.  
* A vector declared closed MUST have exactly the same Gate-key set as the observed projection. A vector declared partial governs only its listed Gate keys.  
* An exception or temporary exclusion MUST be encoded in the governed declaration, not supplied as ad hoc validator behavior.

The check MUST rerun whenever either topology catalog or the declared vector changes.

Acceptance hints: `DEGREE_VECTORS_DECLARED_OK`, `DEGREE_VECTORS_MATCH_OK`, and `DEGREE_VECTORS_NONNEG_INT_OK`.

### **3.2.5 Distinguished sets**

A distinguished set is an optional, named, governed subset of Gate or Channel identities. It is enforced only after an owning artifact declares it.

Each declaration MUST contain:

* `name`, as a non-empty string;  
* `type`, with the value `gates` or `channels`;  
* `members`, as an array governed as a set; and  
* only metadata explicitly permitted by its owning contract.

For `gates`, member identity is the integer Gate ID. For `channels`, member identity is canonical `NN-NN`.

When a distinguished set is declared:

* every member MUST belong to its current closed domain;  
* members MUST be unique by identity and ASCII-sorted by their canonical string projection;  
* a Channel member MUST pass Gate membership, canonical identity, distinct-center, and derived-center checks;  
* a set declared closed MUST contain its complete owned membership; and  
* a set declared partial governs only its listed membership and set semantics.

Acceptance hints: `DISTINGUISHED_DECLARED_OK`, `DISTINGUISHED_DOMAIN_CLOSED_OK`, `DISTINGUISHED_NO_CONFLICTS_OK`, `DISTINGUISHED_ASCII_SORT_OK`, and `DISTINGUISHED_TOPOLOGY_CONSISTENT_OK`.

### **3.2.6 Integration and multiplicity invariants**

* In the current Gate graph, only Gates `10`, `20`, `34`, and `57` have degree `3`; every other Gate has degree `1`. Any other degree vector fails closed.  
* Reducing Channels to unordered center pairs MUST produce 36 total Channel edges. Any expected per-pair multiplicity vector MUST be encoded in a governed artifact or as an explicit closed declaration in `catalog/channels_v1.json`; validation MUST compare the observed vector with that owned declaration.  
* At Channel identity level, the graph is simple: canonical `NN-NN` edges are unique. At center-pair level, the graph is a multigraph and parallel center-pair edges are permitted. Engine mechanics remain per Channel identity.
...
Acceptance hints: `TOPOLOGY_INTEGRATION_DEGREE_OK`, `TOPOLOGY_CENTER_MULTIPLICITY_OK`, and `TOPOLOGY_SIMPLE_GRAPH_OK`.

## **3.3 Domain closure and enums**

All identities and enum values in a current closed domain MUST come from that domain's owning catalog or schema. Unknowns are rejected.

### **Scope**

This subsection applies to the current closed domains in §2: Gate IDs, derived center IDs, Channel IDs and enums, Magic-10 IDs, order and caps, viewer-preference keys, present seed-metadata keys, and narratives identities. Authorities, Profiles, Presets, full-ten seed coverage, and enriched UMS fields are Future-Promotion surfaces. They are not current closed-domain or CI inputs until promoted through a Doc-Delta.

### **Normative rules**

* For a given release, each current catalog defines the complete set of valid identities or enum values in its scope.  
* Every referenced identity or enum value MUST belong to its owning current domain. An unknown value fails closed.  
* Unknown values MUST NOT be silently coerced, normalized, or substituted. Only an explicitly governed input alias may map to a canonical ID, and canonical outputs contain only canonical IDs.  
* Simple IDs are case-sensitive ASCII matching `^[a-z0-9_]+$`. A schema-defined composite identity is an explicit exception and MUST match its owning projection; Channel IDs use canonical `NN-NN` under §3.2.1.  
* A closed-domain addition, removal, rename, or semantic reassignment requires explicit review, a Doc-Delta, dependent validation, and release-identity treatment when the affected bytes are frozen inputs.  
* When order is semantically consumed, the consumer MUST use the owning catalog order. Otherwise the domain is a set and deterministic emissions MUST ASCII-sort by canonical identity.

### **Specific applications**

* Magic-10 categories. Only the ten IDs in §2.3 are valid. Canonical emission order is exactly the order in `catalog/magic10.json` and §2.6.  
* Viewer preferences. The object MUST contain exactly `top_category` and `weights`. `top_category` MUST be one Magic-10 ID, and `weights` MUST contain exactly all ten Magic-10 IDs. Each weight MUST be an integer in `0..100`.  
* Magic-10 caps. `catalog/magic10_caps.json` MUST contain exactly the ten Magic-10 keys. Each row MUST have a non-empty `inputs` array and integer bounds whose current values are `min:0` and `max:100`.  
* Magic-10 seeds. Zero or more rows are allowed. Present keys MUST be a subset of the Magic-10 IDs. Each row MUST contain exactly `template_id`, `seed_version`, `updated_at_utc`, and `checksum_sha256`. There is no current `admin_only` field or exact-ten requirement.  
* Centers. The canonical machine set is `ajna`, `ego`, `g`, `head`, `root`, `sacral`, `solar_plexus`, `spleen`, and `throat`. `g_center` and `ego_heart` are not canonical output IDs.  
* Gates. Gate IDs are exactly the integers `1..64`, each occurring once and mapping to one closed center ID.  
* Channels. Channel IDs are exactly the 36 closed min-first pairs in §2.1. Each row references two distinct Gate IDs and two distinct centers whose set equals the centers derived from those Gates. Values for `circuit_primary`, `substream`, `primary_domain`, `domains`, and `flags` MUST belong to the enums in `schemas/channels_v1.schema.json`; `primary_domain` MUST also be a member of that row's `domains` set.  
* Narratives. Response identities and discriminated output shapes MUST satisfy §3.4. Pack identities remain owned by §2.8 and their manifest contract.

### **Validation mechanics**

* Use a JSON Schema enum for an explicit closed set where feasible.  
* For a reference into another catalog, construct the owner set from the current catalog and validate every reference against it.  
* A constraint outside JSON Schema capability MUST have a fail-closed companion check.  
* Alias acceptance MUST be opt-in and ledger-governed at ingestion. Alias values MUST NOT enter canonical output or create additional primary rows.

### **Current implementation boundary**

The current registry loader does not enforce exact topology cardinalities, exact allowed-key sets, distinct Gate or center pairs, declared-center derivation, all Channel enum domains, exact cap bounds, exact seed-row keys, seed timestamp and checksum formats, or canonical catalog bytes. The current viewer-preferences validator enforces the `top_category` domain and exact ten-key `weights` map, but it accepts booleans through Python's integer type relationship, does not reject additional top-level preference keys, and normalizes with a hard-coded order that places `heat` before `harmony` instead of using the catalog-owned order. These gaps MUST be closed before complete domain-contract conformance may be claimed.

### **Enforcement**

* Domain-closure checks MUST run on every change to a catalog or referencing artifact.  
* An unknown identity or enum value is a hard failure.  
* A closed-set change MUST rerun all dependent validations in the same governed change.  
* Deterministic domain and ordering checks MUST execute under `LC_ALL=C`.

Acceptance hints: `CATALOG_DOMAIN_CLOSED_OK`, `CATALOG_NO_UNKNOWN_IDS_OK`, `PREFS_KEYSET_10_OK`, `MAGIC10_DOMAIN_CLOSED_OK`, `XREF_MEMBERSHIP_OK`, `ALIASES_INPUT_ONLY_OK`, and `ID_CHARSET_POLICY_OK`.

## **3.4 Narratives composer response schema \[Required-Now\]**

The owning schema path is `schemas/narratives.composer.response.v1.json`. It MUST use JSON Schema 2020-12, include stable `$schema` and `$id` values, reject unknown properties, and accept exactly one of the following variants.

### **Text variant**

The object contains exactly:

* `composition_id`: an ASCII string with length `8..128`;  
* `fragment_ids`: an array of strings with at least one item;  
* `pack_sha`: lowercase 64-hex; and  
* `text`: a string of at most 300 characters containing no carriage return.

The Text variant MUST NOT contain `suppressed` or `policy_reason`.

### **Suppressed variant**

The object contains exactly:

* `suppressed`: the boolean constant `true`;  
* `policy_reason`: the string constant `conflict`;  
* `composition_id`: an ASCII string with length `8..128`; and  
* `pack_sha`: lowercase 64-hex.

The Suppressed variant MUST NOT contain `text` or `fragment_ids`.

### **Serialization and validation**

* The schema MUST express the variants as mutually exclusive shapes and MUST reject an object that satisfies neither or combines fields from both.  
* The lowercase digest pattern for `pack_sha`, the length bounds for `composition_id`, the minimum `fragment_ids` cardinality, the `text` length limit, and the prohibition on `\r` MUST be schema-enforced.  
* Canonical JSON under §4 applies whenever a response object is persisted as an artifact. Transport rendering does not create a second JSON serialization contract.  
* Transport headers, empty-body behavior, access control, retention, and logging remain owned by their respective transport, infrastructure, and privacy contracts. Narrative text MUST NOT be copied into evidence or logs merely to prove schema conformance.
...
Source bytes are owned by the catalog paths named in §2. Structural topology shapes are owned by `schemas/gates_v1.schema.json` and `schemas/channels_v1.schema.json`. Executable cross-catalog, Magic-10, manifest, and alias validation is owned by `engine/config/registry_loader.py`. Tests verify those owners; tests are not alternate catalog authorities.

### **Current executable and declarative surfaces**

* `schemas/gates_v1.schema.json` — exact object/row keys, 64 rows, gate range `1..64`, and the nine-center enum using machine IDs `g` and `ego`.  
* `schemas/channels_v1.schema.json` — exact object/row keys, 36 rows, current field types/enums, and uniqueness for `domains` and `flags`.  
...
* `tests/config/test_alias_policy_enforcement.py` and `tests/config/test_config_loader_unknown_ids_fail_closed.py` — current loader regression coverage within the default `pytest.ini` discovery scope.

### **Required validation contract**

A conforming topology change MUST first validate against both current schemas as applicable, then pass the hardened loader. The combined checks MUST reject malformed JSON, missing or additional properties, wrong types or enums, wrong cardinality, duplicate identities, gates outside `1..64`, noncanonical Channel identity, unknown gate/center references, Magic-10 order or caps drift, malformed caps/seed rows, seed keys outside the Magic-10 set, malformed or unsafe manifest paths/structure, and aliases outside the explicit allow-list policy.

`catalog/magic10_caps.json` MUST have exact rows and current integer bounds `0..100`. Present `catalog/magic10_seeds.json` rows MUST have exact keys, valid timestamp shape, and lowercase-64-hex checksum shape. Viewer preferences MUST reject extra top-level keys and booleans/non-integers as weights. Authorities, Profiles, and Presets are excluded until a Doc-Delta adds verified paths, schemas or validators, and consumers.

A failure MUST make the invoking workflow fail. This section defines ownership and behavior, not a GitHub Actions job name. No `catalog_*` workflow/job name is asserted.
...
`CATALOG_SCHEMA_OK`, `CATALOG_NO_ADDITIONAL_PROPS_OK`, `CATALOG_NO_UNKNOWN_KEYS_OK`, `CATALOG_DOMAIN_CLOSED_OK`, `SCHEMA_HYGIENE_OK`, `SCHEMA_ID_STABLE_OK`, `MANIFEST_PATH_SAFE_OK`, `EVIDENCE_INDEX_UPDATED_OK`, `EVIDENCE_INDEX_MIRROR_OK`.

## **8.2 Topology, set, canonical-byte, and projection integrity \[Required-Now\]**

### **Ownership**

This section owns integrity beyond §8.1 shape and domain closure: topology coherence, arrays-as-sets normalization, exact governed bytes, and deterministic derived projections. It does not redefine source catalogs.

### **Current surfaces**

* `tools/generate_registry_report.py` — sole producer of `artifacts/registry/registry_report.json`; loads through the current registry loader, requires closed rails, hashes its consumed catalog bytes, and emits canonical JSON.  
* `tests/config/test_registry_report_determinism.py` — current registry-report two-run coverage in the default `pytest.ini` discovery scope.  
...
After §8.1 passes, integrity validation MUST prove all of the following:

* every Channel has two distinct Gates;  
* its `NN-NN` identity is the zero-padded min-first projection of those Gates;  
* its declared center set is exactly the set derived from the two Gate rows, and the two centers are distinct;  
* arrays declared as sets are deduplicated and deterministically sorted before storage, comparison, and emission;  
* every governed JSON file required by §4 already equals the canonical serializer’s bytes on disk, including exactly one trailing LF;  
* registry-report input paths, counts, and SHA-256 values match current source bytes;  
* the report’s Channel IDs, Gate-center map, center/domain sets, domain counts, Magic-10 projection, and alias policy equal a fresh loader-derived projection; and  
* two regenerations from identical governed inputs are byte-identical.

Manifest completeness and `release_id` remain owned by §§5–6. Human Index, Machine Mirror, and path-proof coherence remain owned by §§8.3 and 8.6. A failure MUST make the invoking workflow fail. This section does not assign CI job names and MUST NOT present `integrity_topology`, `integrity_arrays_as_sets`, or `integrity_canonicalization_compare` as existing workflow/job names.
...
`TOPOLOGY_NO_ORPHANS_OK`, `TOPOLOGY_CHANNEL_DEGREE_2_OK`, `TOPOLOGY_GATE_CENTER_OK`, `TOPOLOGY_CENTER_MULTIPLICITY_OK`, `ARR_SET_NO_CONFLICTS_OK`, `ARR_SET_ASCII_SORT_OK`, `FILE_EQ_CANON_BYTES_OK`, `TWO_RUN_IDENTITY_OK`, `ENV_LC_ALL_C_OK`.
...
Governed evidence artifacts and their sibling path-proof transcripts (\\.path\_proof.txt) MUST NOT be ignored by .gitignore. Governed locations are expected to be tracked; using .gitignore to hide governed artifacts or their path-proofs is invalid and should be treated as a QA failure.
...
Each Mirror record MUST contain the following required core keys. Optional keys are limited to the extension below; all other keys are rejected:

* artifact\_key — string.  
* role — string. The Machine Mirror self-record MUST use self\_record.  
* sha256 — lowercase 64-hex string.  
* size\_bytes — non-negative integer.  
* produced\_at\_utc — UTC ISO-8601 string.  
* discovered\_physical\_path — governed repo-relative path.  
* proof\_anchor — governed repo-relative sibling path-proof path.

### **Bundle-aware extension (evidence bundles, manifests, and change-container metadata)**

The minimum record schema above remains normative for all Mirror records. This section extends it to cover bundle artifacts, their manifests, and Epic or CRD metadata without changing the core field set or canonical JSONL discipline.
...
* crd\_id — the stable identifier for the owning CRD; semantics and work-record lineage live in PF30 CRD Register and Glow QA Guide (titles-only).  
...
* tokens — an optional non-empty array-as-set of acceptance-token names. Values MUST be deduplicated and ASCII-sorted. For HDE-EPIC020 records, every value MUST be drawn from the EPIC020 acceptance roster.

A record MAY contain epic\_id or crd\_id, but MUST NOT contain both. Existing historical and active epic records retain epic\_id and MUST NOT be migrated merely to add CRD compatibility.

Before the first affected CRD enters QA, the Machine Mirror schema and every serializer or validator enforcing this allowed-key set MUST accept crd\_id, enforce the epic\_id/crd\_id mutual-exclusion rule, and preserve valid historical and active epic\_id records.

Bundle records use only the required core keys plus applicable metadata keys above. No additional bundle-specific top-level keys are allowed.

#### **Bundle artifacts and manifests**

A bundle artifact is a textual JSON or JSONL file that groups multiple logical evidence members. Its Mirror record’s artifact\_key identifies the governed bundle family, and discovered\_physical\_path points to the bundle file.

A bundle manifest enumerates the bundle’s members and is a separate governed artifact with its own Mirror record, artifact\_key, and discovered\_physical\_path.

For selected bundle-based families, the Human Evidence Index and Machine Evidence Mirror track the bundle file and its manifest instead of listing every internal member as a separate row. Internal member content is addressed by the manifest.
...
Other epics or CRDs may introduce similar bundle and manifest patterns with different record\_type values and the applicable epic\_id or crd\_id value. PF12 remains the single home for the allowed key set and canonical JSONL constraints, while change-container meaning is routed by title to HDE-Governance, HDE-Phased Epics for epics, PF30 CRD Register for CRDs, and Glow QA Guide.

#### **Path-proof semantics for bundles**

Each bundle artifact and its manifest MUST have governed sibling path-proof transcripts (\\.path\_proof.txt and \\.path\_proof.txt) stored alongside each file, whose path, sha256, and size\_bytes match the bundle/manifest file and the corresponding Mirror record values.
...
This extension does not introduce hash-only or index-only families. Any future move toward hash-only evidence without on-disk member payloads remains a Future-Promotion and requires an explicit reconciliation Doc-Delta.
...
The mirror MUST contain exactly one self-record with:

* artifact\_key equal to `index.machine_mirror`;  
* role equal to `self_record`;  
* discovered\_physical\_path equal to `artifacts/evidence_index.jsonl`; and  
* proof\_anchor equal to `artifacts/evidence_index.jsonl.path_proof.txt`.

For this self-record:

* sha256 MUST equal the SHA-256 digest of the canonical JSONL body excluding the self-record line.  
* size\_bytes MUST equal the byte length of the complete mirror file including the self-record line.

The sibling path-proof MUST record:

* path equal to `artifacts/evidence_index.jsonl`;  
* sha256 equal to the SHA-256 digest of the complete mirror file;  
* size\_bytes equal to the complete mirror byte length; and  
* mirror\_body\_sha256 equal to the self-record’s sha256.

The path-proof sha256 therefore differs from the self-record sha256: the former proves the complete file, while the latter proves the non-self body.

All non-self records continue to bind sha256 and size\_bytes to the referenced artifact at discovered\_physical\_path, with matching sibling path-proofs.

### **Field order and write discipline (merge-blocking)**

Every record MUST serialize all present keys in ASCII ascending order. For a record containing only the required core keys, the order is exactly: artifact\_key, discovered\_physical\_path, produced\_at\_utc, proof\_anchor, role, sha256, size\_bytes. Optional metadata keys appear at their ASCII-sorted positions.
...
produced\_at\_utc records when the evidence was logically produced. It is part of the Mirror record and sibling path-proof and is the primary evidence-ordering timestamp.

mtime\_utc is capture-time provenance stored in the sibling path-proof. A newly produced proof may seed it from the artifact’s observed filesystem modification time. A later clone, cache restore, or CI checkout MUST NOT compare it with that checkout’s local filesystem mtime.

Both timestamps MUST use the governed UTC shape. Their values provide provenance context and do not replace path, hash, size, canonical-field, or companion-linkage integrity.

### **Path-proof transcript schema (governed artifacts)**

Naming (MUST). The sibling path-proof transcript for a governed artifact MUST be named \\.path\_proof.txt, where \\ includes the artifact’s full filename including extension. Example: audit/gates/determinism/env\_pins.log → audit/gates/determinism/env\_pins.log.path\_proof.txt (not env\_pins.path\_proof.txt).

For every governed artifact in §8.6, the path-proof transcript MUST be a co-located sibling file named \\.path\_proof.txt. It MUST describe exactly one artifact and follow a stable, line-oriented schema.
...
* mtime\_utc — UTC ISO-8601 timestamp (e.g. YYYY-MM-DDThh:mm:ssZ) recording capture-time provenance (see “mtime\_utc semantics” below).  
...
* the transcript’s path field matches the mirror’s discovered\_physical\_path,  
...
mtime\_utc records the artifact filesystem modification time observed when the path-proof is first produced or materially refreshed. It MUST use `YYYY-MM-DDThh:mm:ssZ` with no fractional seconds.

A later clone, cache restore, archive extraction, or CI checkout MUST NOT compare mtime\_utc with clone-local `stat().st_mtime`. Git does not preserve that metadata. An unchanged valid proof MUST NOT be rewritten or re-timestamped solely to match local filesystem metadata.

#### **Interaction with produced\_at\_utc**

produced\_at\_utc records when the evidence was logically produced. It MUST use the same governed UTC shape. Family-specific generated\_at\_utc or non-backdating rules remain additive where expressly defined.

#### **Integrity semantics**

The primary integrity proof is the coherent match among:

* the governed repo-relative path;  
* the artifact’s canonical bytes;  
* sha256 and size\_bytes in the Machine Mirror;  
* sha256 and size\_bytes in the sibling path-proof;  
* the required canonical fields; and  
* the proof\_anchor and companion linkage.

mtime\_utc and produced\_at\_utc provide provenance context; they do not replace those integrity bindings.

#### **Alignment with tools and QA**

Evidence tooling and CI MUST:

* recompute sha256 and size\_bytes from canonical artifact bytes;  
* preserve valid existing timestamps when the governed artifact and proof fields are unchanged;  
* seed a newly produced mtime\_utc from the observed filesystem time;  
* validate UTC shape, path, hash, size, required fields, and companion linkage; and  
* never use later-checkout filesystem metadata as evidence input.

Tests MUST enforce these same portable semantics.

#### **Change control**

Any change to mtime\_utc or produced\_at\_utc semantics requires:

* a PF12 Doc-Delta (§9);  
* synchronized updates to PF19 and PF10; and  
* synchronized updates to the evidence tooling and tests.
...
* artifact\_key equals the Human Index artifact\_key.  
* discovered\_physical\_path equals the Human Index discovered\_physical\_path.
...
* Its sibling \\.path\_proof.txt path-proof transcript (the proof\_anchor target).  
...
Process and PR workflow (who runs which command and when) remains single-homed in Change Process Guide; this section pins file-level ordering and artifacts that MUST be updated together.
...
* mtime\_utc in the path-proof remains capture-time provenance and MUST have valid UTC shape. It is not compared with a later checkout’s filesystem mtime. mtime\_utc may be earlier than generated\_at\_utc, but the governed path, sha256, size\_bytes, canonical fields, and companion linkage MUST still match.

For governed families that include both generated\_at\_utc and produced\_at\_utc, provenance correctness for a refresh requires:

* Mirror produced\_at\_utc to be no earlier than payload generated\_at\_utc; and  
* path-proof produced\_at\_utc to match Mirror produced\_at\_utc exactly.

Integrity gates continue to rely primarily on governed-path, sha256, size\_bytes, canonical-field, and companion-linkage equality. Timestamp checks are additive: valid UTC shape, Mirror/path-proof produced\_at\_utc equality, and the explicit non-backdating rule relative to payload generated\_at\_utc.
...
The artifact contains a header block, exactly one blank line, and one LF-terminated canonical JSON refusal body.

Index it in both `docs/evidence/INDEX.json` and `artifacts/evidence_index.jsonl` in the same PR. Its required sibling path-proof is `artifacts/proofs/ops_refusal_proof.txt.path_proof.txt`, and the Mirror proof\_anchor MUST equal that exact path.

Policy and token semantics live in HDE-Governance.

#### **File format (exact)**

##### **Header block**

One header per line in the form `<lower-case-name>: <verbatim-value>`. No blank lines may occur inside the header block.
...
Other headers may appear only when permitted by the owning transport policy. Header order is preserved as captured.
...
The body is one deterministic, numeric-free typed-refusal JSON object serialized canonically under §4 and terminated by exactly one LF. Exact refusal-envelope semantics remain owned by HDE-Governance and HDE-CLI-API-Vendor-Ref.

The refusal-log six-key allow-list applies to logs, not to this captured response body.

#### **Mirror linkage**

The Mirror record uses artifact\_key `ops.rails_refusal`, role `proof`, discovered\_physical\_path `artifacts/proofs/ops_refusal_proof.txt`, and proof\_anchor `artifacts/proofs/ops_refusal_proof.txt.path_proof.txt`.

#### **Validation checks (CI)**

* Required headers are present and lower-case.  
* Forbidden headers are absent.  
* Exactly one blank line separates header block and body.  
* Body is one canonical, numeric-free typed-refusal JSON object.  
* File ends with exactly one LF.  
* Checks run under `LC_ALL=C`, `LANG=C`, and `TZ=UTC`.
...
{"code":"rails_closed","error":"rails remain closed","ok":false,"schema":"rails_closed"}
```
...
## **8.4 Reserved \[Tracking\]**

No recoverable or current normative §8.4 contract is asserted. The number is reserved so §8.6 onward retains stable identities and cross-references. Future use of §8.4 requires a Doc-Delta that identifies the owner, paths, schemas, producer, tests, evidence/indexing posture, migration impact, and every affected cross-reference. Reserved numbering is not evidence that missing prose exists.

## **8.5 Registry report \[Required-Now\]**

### **Single home and identity**

The governed registry report lives at exactly `artifacts/registry/registry_report.json`.

Its Human Evidence Index and Machine Mirror identity is `registry.registry_report`; its `record_type` is `registry_report`, its `schema_version` is `1.0`, and its Mirror role is `snapshot`. PF09 phase checklists consistently require this file to be canonical JSON, kept in sync, and mirrored.

### **Producer and execution boundary**

`tools/generate_registry_report.py` is the sole producer. Generation MUST pass through `engine/config/registry_loader.py`, MUST require closed rails, and MUST write canonical JSON. A hand-edited report is not authoritative.

### **Input contract**

`inputs.catalogs` describes only current producer inputs:

* `channels_v1` — path, row count, and SHA-256 for `catalog/channels_v1.json`;  
* `gates_v1` — path, row count, and SHA-256 for `catalog/gates_v1.json`;  
* `magic10_order` — path, SHA-256, and ordered keys for `catalog/magic10.json`;  
* `magic10_caps` — path and SHA-256 for `catalog/magic10_caps.json`; and  
* `magic10_seeds` — path and SHA-256 for `catalog/magic10_seeds.json`.

Magic-10 seeds are a validated subset of the Magic-10 domain. The report MUST NOT claim all ten seed entries unless a later source contract actually requires and supplies them.

### **Output contract**

The top-level object contains exactly `schema`, `generated_at_utc`, `inputs`, `artifacts`, and `notes`, with `schema: "registry_report.v1"`. `artifacts.registry` contains:

* `channel_ids` — canonical sorted primary Channel IDs;  
* `gate_centers` — Gate-number strings mapped to machine center IDs;  
* `centers` — the closed sorted center set;  
* `domains` — the closed sorted domain set derived from Channel rows;  
* `domain_counts` — deterministic counts derived from Channel-domain membership;  
* `magic10` — ordered keys, caps, and currently present seed metadata; and  
* `alias_policy` — explicit mode and sorted alias map.

The producer serializes with recursive sorted keys, compact separators, and exactly one final LF. `generated_at_utc` is deterministic: use `SOURCE_DATE_EPOCH` when supplied; otherwise retain the existing report timestamp; if neither exists, use the producer’s fixed epoch fallback. Two regenerations from identical inputs MUST be byte-identical.

### **Indexing and companions**

The same change that modifies report bytes updates `docs/evidence/INDEX.json`, `artifacts/evidence_index.jsonl`, and `artifacts/registry/registry_report.json.path_proof.txt`, together with the hash and size bound by those records. The current family has no separate `.sha256` sidecar requirement. Preserve the existing `registry.registry_report` identity rather than minting a second key.
...
* Detailed PR/workflow process is defined in Change Process Guide (titles-only).  
...
* stable PF, ADR, epic, CRD, task, acceptance-criterion, test, schema, or artifact identifiers;  
...
* `PRESENTER_READER_CLI_PARITY_V1` — `presenter Reader and CLI parity bytes`. Reader-versus-CLI presenter parity sample. Example path: `artifacts/presenter/reader_cli_parity.bytes`. Mirror role: snapshot.  
* `PRESENTER_SHOWCOMPAT_AB_BYTES_V1` and `PRESENTER_SHOWCOMPAT_BA_BYTES_V1` — `presenter AB and BA identity bytes`. Example paths: `artifacts/presenter/showcompat_ab.bytes` and `artifacts/presenter/showcompat_ba.bytes`. Mirror role: snapshot.

Acceptance hints (names-only). These families support CLI\_SHOWCOMPAT\_CANON\_OK, TWO\_RUN\_IDENTITY\_OK, COMPOSITE\_ABBA\_IDENTITY\_OK, and PREIMAGE\_RECOMPUTE\_OK. PF12 binds tokens to artifacts by name and path only and does not redefine token semantics.
...
* Path: JSON file under artifacts/config\_bundles/, exact filename pinned by the bundle generator and tests.  
* Path-proof: a sibling \\.path\_proof.txt transcript stored alongside the bundle file in the same directory. **Mirror record.** artifact\_key:"config\_bundle.be", role:"snapshot", discovered\_physical\_path equal to the bundle path, with sha256, size\_bytes, produced\_at\_utc, and proof\_anchor matching the bundle’s canonical bytes and path-proof as required by §8.3 and §8.15.
...
`api-reference/openapi.json` MUST NOT define vendor bytes, schemas, endpoint routes, request shaping, response mapping, or architecture conformance unless its HumanDesignAPI domain, title, server, and path-family ownership are validated and recorded in the governed validation artifacts. Documentation-discovery files such as `llms.txt` and `llms-full.txt` MAY be recorded only as source-inventory entries. They MUST NOT create:
...
YAML, text, Markdown, CSV, body, and log artifacts in this family MUST be UTF-8 and LF-terminated. The Human Evidence Index and Machine Evidence Mirror MUST each carry exactly one binding for every promoted path. Mirror records MUST use the defined artifact\_key and point proof\_anchor to the sibling path-proof. When this family changes, the following MUST be refreshed coherently in the same change:
...
##### **Additional optional per-check and bootstrap outputs (generic, HDE-EPIC027, and HDE-EPIC028)**
...
* `audit/qa/<epic-id>/00_meta/deferred_scope_posture.md.sha256`: Optional sibling integrity sidecar for `audit/qa/<epic-id>/00_meta/deferred_scope_posture.md`, produced by the check that generated the posture record.  
...
The `<epic-id>` segment is the canonical identifier used in §8.17: lower-case ASCII and hyphenated, for example `hde-epic021`. A differing semantic identifier, for example `HDE-EPIC021`, MAY be recorded in Machine Mirror metadata via epic\_id. The matrix MUST contain no secrets.
...
For each token row in the matrix, the artifact MUST record at least: token\_name: the QA token’s canonical name, as defined in HDE-Governance and or Glow QA Guide or an approved doc delta (titles-only). owner\_pf: the PF document (and optionally section) that owns the token’s semantics (titles-only). evidence\_artifacts: one or more governed artifacts associated with the token (artifact\_keys and or discovered\_physical\_path entries), drawn from families listed in §8.6 and other PF12 sections. `qa_root_logs`: QA log paths under `audit/qa/<epic-id>/` that demonstrate QA harness runs relevant to this token (current-state preferred; retained history optional). ci\_tests\_jobs: CI test modules and or jobs that enforce this token under closed rails (names only). Additional columns such as Status or Notes MAY be present for human use; their contents are not governed by PF12 beyond canonical text formatting and governed-path rules.
...
* {"generated\_at\_utc":"YYYY-MM-DDThh:mm:ssZ","endpoints":\["\\","\\"\],"envelope\_keys":\["reader\_version","eligible","categories","meta","release\_id","idempotence\_hash"\]}
...
* {"generated\_at\_utc":"YYYY-MM-DDThh:mm:ssZ","endpoints":\[{"path":"\\","method":"\\","internal":true,"classification":"\\","env\_gate":"\\","a7\_eligible":false,"blueprint\_module":"\\","rails\_profile":"\\"}\]}
...
Human Evidence Index (`docs/evidence/INDEX.json`): add a titles/paths-only entry for `sbom/cyclonedx.json` and `sbom/cyclonedx.json.sha256` in the same PR that adds or changes them.
...
Routing (titles-only). Provenance policy and release workflow: Change Process Guide; HDE-Governance §2.0 tokens and evidence rules.
...
* `{"after_304":{"no_body":true,"omits_content_length":true,"omits_content_type":true,"seen_after_prior_get":true},"env_gate":{"gated_ok":true,"proof_path":"artifacts/proofs/endpoints_env_gate_proof.log"},"etag":{"encoding_invariance_ok":true,"identity_etag":"\"<strong-etag>\"","tested_encodings":["br","gzip","identity"]},"get_200":{"body_sha256":"<64-hex>","captured_at_utc":"YYYY-MM-DDThh:mm:ssZ","content_type":"application/json; charset=utf-8","etag":"\"<strong-etag>\""},"head_200":{"content_length_equals_identity":true,"content_type_equals_get":true,"no_body":true},"route_path":"<Catalog title or identifier of the success route>","vary_flags":{"accept_encoding":true,"authorization":true}}`
...
* Include seed metadata for each category currently present in `catalog/magic10_seeds.json`. Present keys MUST be a subset of the closed Magic-10 domain. Each present row contains:  
  * `template_id` — string.  
  * `seed_version` — integer or string version identifier.  
  * `updated_at_utc` — UTC ISO-8601 timestamp string.  
  * `checksum_sha256` — lowercase 64-hex identity for the referenced seed content.

The config MUST preserve subset semantics and MUST NOT synthesize missing category rows. A future full-ten requirement must first change §2.7 and the owning implementation through Doc-Delta.
...
* Each concrete frontend bundle file MUST have a sibling path-proof transcript named \\.path\_proof.txt stored alongside the bundle file, whose path, sha256, size\_bytes, mtime\_utc, and produced\_at\_utc match the bundle’s canonical bytes and mirror record.  
...
* Repo docs MUST reference PF documents by title only (for example, “HDE-Schemas and Artifacts”, “Glow QA Guide”, “HDE-Phased Epics”, “Change Process Guide”) and MUST NOT inline or restate canonical schemas, Evidence Index field sets, or acceptance token definitions.  
...
* Live QA artifacts are easy to locate and reason about across epics, CRDs, and attempts.  
* Evidence promoted to governed status can be indexed and mirrored consistently.  
* The QA process can rely on predictable naming without re-specifying it per governed change container.

Change-container identity (normative).

This generic Live QA layout supports both Epic/PF09 and CRD/PF30 work.

* For an epic, `<change-id>` is the existing `<epic-id>`, and `EPIC_QA_ROOT` remains `audit/qa/<epic-id>/`.  
* For a CRD, `<change-id>` is the stable CRD ID recorded in PF30 using the lower-case ASCII, hyphenated directory form required by PF12; its governed QA root is `audit/qa/<change-id>/`.  
* Wherever the generic provisions of §8.17 use `<epic-id>`, `EPIC_QA_ROOT`, “epic-level,” or “per-epic,” the CRD lane substitutes `<change-id>`, the CRD QA root, “CRD-level,” or “per-CRD.”  
* Named HDE-EPIC-specific families and paths remain epic-specific. Existing historical and active epic roots remain valid and MUST NOT be migrated merely to add CRD compatibility.

Before the first affected CRD enters QA, every schema, validator, generator, manifest producer, or helper that accepts only epic identity or `EPIC_QA_ROOT` MUST be identified and updated through the authorized change lane. The updated surfaces MUST preserve existing epic inputs while accepting the applicable CRD identity and QA root.
...
* Re-running a check MUST reuse stable check-scoped paths under audit/qa/\\/checks/\\/ and MUST NOT create a new run root.

Tools MUST NOT infer run state by enumerating subdirectories under audit/qa/\\/. The manifest is the authoritative index of current-state step evidence.
...
Live QA Plans MUST NOT include any required deliverable whose sole purpose is “PF23 consult capture.” Planning consult capture is planning-time only and is not a governed member of the audit/qa/\\/ evidence layout.
...
* The `EPIC_QA_ROOT` directory name and all structural child directories, including `00_meta/`, `checks/`, `results/`, `snapshots/`, `closeout/`, and `remediation/`, MUST use lower-case ASCII.  
...
audit/qa/\\/checks/\\/primary.log

The directory checks/\\/ is the check-scoped home for the check. The \\ directory name MUST match the check\_id used for manifest keying (see §8.17.1).
...
Governed Live QA evidence files under audit/qa/\\/ MUST NOT be empty:
...
* live under audit/qa/\\/results/ or a check-scoped subdirectory  
...
Separate snapshot files under audit/qa/\\/ MAY be produced (recommended: under 00\_meta/ for baseline and under results/ for per-check snapshots), but they are optional and MUST NOT be required for closure unless explicitly promoted as acceptance-decisive governed evidence.
...
audit/qa/\\/00\_meta/codespaces\_snapshot.json

Run-id discipline note (normative).

`run_id` MAY be retained as nullable provenance metadata inside governed records. It MUST NOT select directories, key evidence, change canonical paths, alter correctness, or create a second indexed copy. The epic-level current-state snapshot is the only governed Codespaces snapshot home.
...
* tool\_versions — array of objects treated as a set (dedupe \+ ASCII-sort by tool):  
  * each item MUST be { "tool": \\, "version": \\ }  
...
* env\_presence — array of objects treated as a set (dedupe \+ ASCII-sort by name):  
  * each item MUST be { "name": \\, "present": \\ }  
...
Section 0.4 is the single home for Doc-Delta triggers. A change requires a Doc-Delta when it changes any current catalog or closed-domain contract, promotes a Future-Promotion surface, changes a schema or executable validation contract, changes canonical bytes or serialization, changes a frozen input or Freeze-Pack Manifest contract, changes governed evidence or parity rules, or changes release identity under §0.4.

An editorial-only reorganization does not require a Doc-Delta when it changes no governed catalog, schema, domain posture, validation rule, canonical-byte rule, artifact identity, canonical path, release contract, or normative requirement.

### **Required record content**

Every Doc-Delta MUST state:

* a stable Doc-Delta ID, date, and author;  
* its scope and every affected catalog, schema, and governed artifact by title and repository-relative path;  
* the change summary and rationale;  
* the current-domain or Future-Promotion impact;  
...
### **Same-change coupling**

The Doc-Delta MUST accompany the change that creates the governed effect. Every required artifact, validation, release-identity, Human Evidence Index, Machine Evidence Mirror, checksum, sentinel, and path-proof update MUST land in the same finalized change.

The Doc-Delta records intended and observed change state. Its presence does not by itself prove that implementation exists, validation passed, evidence is coherent, a new `release_id` was computed, or the change landed.
...
Complete every applicable field and attach the Doc-Delta to the same change that introduces the governed effect. Reference other PF specifications by title only.

Keep the Human Evidence Index (`docs/evidence/INDEX.json`) and the Machine Evidence Mirror (`artifacts/evidence_index.jsonl`) in the same change. Update the Human Evidence Index hash sentinel and Machine Evidence Mirror checksum whenever their governed bytes change.
...
doc_delta:
  ...
  targets:
    catalogs_changed:
      - title: "<catalog title>"
        ...
        operation: "<add|remove|rename|move|modify>"
    schemas_changed:
      - title: "<schema title>"
        path: "<repository-relative path>"
        operation: "<add|remove|rename|move|modify>"
  ...
  release_identity:
    frozen_input_changed: "<true|false>"
    manifest_changed: "<true|false>"
    release_id_expected_change: "<true|false>"
    prior_release_id: "<value|not-applicable>"
    computed_release_id: "<value|[OPEN]|not-applicable>"
  ...
  validation:
    - behavior: "<required behavior>"
      entrypoint: "<verified command or workflow check>"
  ...
  evidence_updates:
    - title: "<evidence title>"
      path: "<repository-relative path>"
      operation: "<add|refresh|move|remove>"
  ...
  index_and_mirror:
    ...
    parity_status: "<pass|fail|not-run|blocked>"
  ...
  open_decisions:
    - id: "<decision id>"
      description: "<smallest unresolved decision>"
      owner: "<owner>"
      next_step: "<next step>"
      status: "open"
  ...
  change_log_entry: |
    <Summarize the landed change, affected titles and paths, release_id impact,
    validation evidence, and Human Evidence Index/Machine Evidence Mirror status.>
```

### **Submission checklist**

* Every changed target is listed by title, path, and operation.  
* Current-domain and Future-Promotion effects are distinguished.  
* Frozen-input, manifest, and `release_id` impact is explicit.  
* Every required behavior names a verified executable entrypoint, result, and evidence path.  
* Human Evidence Index, Machine Evidence Mirror, checksum, sentinel, and path-proof updates are synchronized where required.  
* Acceptance-token names come from HDE-Governance; no token semantics or CI job names are invented locally.  
* Every unresolved item is `[OPEN]` and includes an owner and next step.  
* The change-log entry reports only the state proven by the landing change.

## **9.3 Acceptance to land**

All required behaviors MUST pass through verified executable entrypoints on the same finalized change. The Human Evidence Index and Machine Evidence Mirror MUST be synchronized. A new `release_id` MUST be recorded when frozen inputs or manifest bytes change.

### **Preconditions**

The landing change MUST bind its concrete commands, workflow checks, results, and evidence to every applicable requirement in:

* §3 for catalog schemas, executable validation, topology, and domain closure;  
* §4 for canonical serialization, arrays-as-sets normalization, locale, and determinism;  
* §§5–6 for the Freeze-Pack Manifest, exact-byte bindings, and `release_id`;  
* §§8.1–8.2 for schema, topology, set, canonical-byte, and Registry Report integrity; and  
* §§8.3 and 8.6 for Human Evidence Index, Machine Evidence Mirror, parity, checksum, sentinel, and path-proof coherence.

The landing change MUST also prove that both current topology schemas validate their current catalogs, governed negative fixtures fail closed, required Channel and closed-domain checks pass, the Registry Report regenerates identically twice from finalized inputs, and every required check is included in the actual default CI or test discovery used by the landing workflow.

PF12 does not treat an illustrative token, behavior label, script name, or prose label as an existing CI job. The Doc-Delta and validation report MUST identify the executable entrypoints that establish each required behavior.

### **Release handling**

If a frozen input or manifest byte changes, recompute `release_id` under §6.2 and record the new value in the governed Manifest Snapshot evidence and the change-log entry.

If no frozen input or manifest byte changes, verify and record that the prior `release_id` remains valid. Do not infer unchanged release identity from an editorial label or an unchanged version string.

### **Environment and determinism**

Run generation and validation with `LC_ALL=C`, `LANG=C`, and `TZ=UTC` as required by §4.3. Required two-run identity checks MUST use the same finalized inputs.

### **Acceptance-token binding**

When a landing change claims acceptance tokens, every name MUST match HDE-Governance and every claim MUST bind to the governed evidence that establishes its registered semantics. PF12 does not redefine token semantics or maintain a speculative minimum-token roster in this section.

### **Failure policy**

A failed, blocked, or unexecuted required behavior; an invalid or inconsistent manifest; a missing required Doc-Delta; unsynchronized Human Evidence Index or Machine Evidence Mirror bytes; a missing required checksum, sentinel, or path-proof; or an inconsistent `release_id` blocks the governed change from landing.

## **9.4 Refresh landing record \[Required-When-Landed\]**

The change that lands the complete catalog, schema, validation, manifest, evidence, and legacy-reference refresh MUST include a refresh record. The record MUST NOT state or imply that the refresh or its companion remediation has landed until the finalized change supplies the required implementation, validation, evidence, and release-identity proof.

The landed record MUST identify:

* the completed Doc-Delta;  
* every affected catalog, schema, manifest, evidence, and reference artifact by title and path;  
* the validation report and verified executable entrypoints;  
* the prior and new `release_id` when frozen inputs or manifest bytes changed;  
* the synchronized Human Evidence Index, Machine Evidence Mirror, checksum, sentinel, and path-proof updates;  
* proof that the legacy UMS enrichment source was preserved as a reference-only artifact before its embedded canonical copy was removed; and  
* the landing pull request and commit.
