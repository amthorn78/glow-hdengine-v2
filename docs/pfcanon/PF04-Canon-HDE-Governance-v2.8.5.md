...
**Last Update Gate:** redlines-PF04-Canon-HDE-Governance-v2.8.3-from-PF10-HDE-Build-Notes-v12.9  
...
* Acceptance gates (A3 / A4 / A7 and Epic-level and CRD-level gates).  
...
When any golden or artifact path changes, the Evidence Index and mirror **must** be updated in the same PR/commit that changes those items, with a matching entry under the change-management rules in **PF06-Canon-Change-Process-Guide**.
...
This governance document defines **how the HD Engine is built, validated, and released** under explicit Epic and CRD gates.
...
* Each CRD functions as a **governed change gate**: a bounded change container that may include one or more PRs, Ops tasks, or both and that remains accountable through its CRD identity and PF30 record.  
* Governance, validation, and operations are inseparable: an Epic or CRD **closes** only when its applicable governance and evidence gates pass.  
...
* **Acceptance gates (A-gates and Epic/CRD gates).**  
  * A3 / A4 / A7 enforce determinism, Reader↔CLI parity, and transport correctness.  
  * Every Epic and CRD uses the same applicable internal criteria: AB↔BA parity, two-run identity, canonical JSON discipline, and A7 transport compliance.  
  * Governance and testing during an Epic or CRD use the same applicable binary proofs required for release acceptance.  
...
* **Change process and work registers.**  
  The **Change Process Guide** owns the common lifecycle and routing for both governed change lanes. Epic-originated work uses Epic identity and **HDE Build Checklist** accountability. CRD-originated work uses CRD identity and **HDE CRD Records** accountability. This document owns authority, approval boundaries, exact-source acceptance and claim separation, and the approved-ADR requirement for canon-changing CRDs.
...
**Claim separation.** Repository presence, CI result, QA verdict, operator or role acceptance, and completion or closure are distinct states and MUST NOT be collapsed. Landing a commit on `main`, including an operator-authored direct push, establishes repository reality only. It does not by itself establish CI PASS, QA PASS, acceptance, PF09 completion, PF30 status movement, OPS completion, deployment, release readiness, epic or CRD closure, or production readiness.
...
* **MACHINE\_MIRROR\_UPDATED\_OK** — The `index.machine_mirror` self-record in the machine JSONL mirror exists and is current: its `sha256` field matches the canonical JSONL body excluding the self-record line; its `size_bytes` field matches the complete `artifacts/evidence_index.jsonl` file including the self-record line; and the sibling path proof and checksum sentinel bind the complete-file SHA-256. (Owned: Evidence & Artifacts)  
...
1. **Human index and hash sentinel are governed artifacts with governed path-proofs.**  
   ...
   1. `docs/evidence/INDEX.json` and `docs/evidence/INDEX.json.path_proof.txt`  
   2. `docs/evidence/INDEX.sha256` and `docs/evidence/INDEX.sha256.path_proof.txt`  
...
3. **Machine mirror self-record separates body-hash and complete-file bindings.**  
4. The `index.machine_mirror` self-record must represent the current `artifacts/evidence_index.jsonl` file. Its `sha256` must match the canonical JSONL body excluding the self-record line, and its `size_bytes` must match the complete Mirror file including the self-record line. Its `proof_anchor` must point to the co-located path-proof for the Mirror file; the sibling path proof and checksum sentinel must bind the complete-file SHA-256.  
5. **Remediation posture for evidence drift (merge-blocking).**  
...
* Governance semantics.  
  ...
  * **KISS required outputs (current-state):**  
    ...
    * if the manifest bytes change as part of that refresh, the co-located \`audit/qa/\\/qa\_step\_logs\_manifest.json.path\_proof.txt\` MUST be refreshed from the new manifest bytes before the step is treated as complete.  
...
Evidence/mirror hygiene (same-PR, canonical JSONL, unknown-key reject, `proof_anchor`): **HDE-Schemas & Artifacts** / **Change Process Guide**.  
...
**Required-check eligibility and lane placement.**

A CI job, required status check, or merge-blocking mechanical rule MUST protect a current, continuing product or delivery risk. Its justification MUST identify the current product behavior or delivery condition protected, the concrete failure detected, the material harm prevented, why the protection remains necessary without reference to an originating epic or closeout, and why CI or merge blocking is proportionate. If those points are not established, the rule MUST NOT be required PR CI.

Product and integration checks belong in required PR CI only when justified. Release-only and security-bound controls belong in release or security automation and gate only that boundary. Deep QA and audit capture belong in bounded QA or audit lanes. Epic-closeout preparation belongs in isolated closeout automation and MUST NOT become required PR CI unless a component independently qualifies for another lane.

Logs, test reports, coverage, security findings, SBOMs, build provenance, release attestations, checksums, and comparable evidence MAY be natural outputs of genuine product, build, security, compliance, release, deployment, or operational controls. Evidence, token, index, mirror, manifest, hash, path-proof, receipt, audit, or closeout requirements alone do not authorize required PR CI. A permanent check MUST stand independently of an epic’s identity and administrative completion; its name does not establish its lane.

Required CI MUST validate committed source rather than generate canonical results that must be written back into tracked source to make the same candidate pass. Existing and proposed controls are subject to the same continuing-risk, lane-placement, proportionality, determinism, actionability, and maintainability tests. Mixed jobs MUST preserve legitimate protection while administrative work is removed, separated, or relocated, and change-aware execution MUST preserve reliable required-check semantics.
...
### **4.3.9 Automated CI cost stewardship**

CI budget requirements MUST use durable repository-owned automation or an authorized repository setting. Manual push-count limits, agent self-reporting, operator pause gates, exception narratives, and transient or implementation reports are not CI budget enforcement, acceptance conditions, remediation tasks, or approval blockers. There is no authoritative five-update threshold. Push counts MAY be inspected to identify structural inefficiency, but a count alone does not establish a violation or require Product Owner authorization.

Automated CI cost stewardship MUST:

* Prevent, separate, or narrow duplicate equivalent full-suite execution for one ordinary pull-request head unless event paths protect materially distinct boundaries.  
* Cancel, short-circuit, or avoid expensive work for a superseded pull-request head whenever that can be done safely without losing required output or weakening another real boundary.  
* Select expensive work deterministically by event and changed surface while preserving a stable required-check conclusion when internal work is inapplicable.  
* Keep release-only, security-only, deep QA, audit, and closeout work out of ordinary pull-request execution unless it independently protects that merge boundary.  
* Consolidate, reuse, cache, narrow, or remove repeated checkout, dependency installation, environment construction, test collection, validation, matrix expansion, and artifact transfer when an equivalent safe deterministic mechanism exists.  
* Preserve source identity, provenance, invalidation conditions, trust boundaries, and failure behavior for caches, shared outputs, and reused artifacts.  
* Ensure that a summary, aggregator, no-op, conditional, or skipped job reports success only when every applicable protection passed or was deterministically proven inapplicable.  
* Ensure that the exact final merge candidate receives every applicable required protection.

A green conclusion MUST NOT conceal a failed, missing, ambiguous, or unexecuted applicable check. Cost improvement MUST be verifiable from workflow topology and observable execution behavior without weakening exact-head validation, bypassing branch protection, leaving required status pending, or misclassifying affected changes.

A material CI-budget finding MUST identify the avoidable hosted execution, the repository or authorized-setting surface that can control it, the automated behavior required, the protection that must remain, and a method for verifying the resulting execution shape. If no durable automated correction exists within authorized scope, the observation MUST NOT be converted into report-only remediation or an acceptance blocker.

These rules do not authorize workflow edits, repository-setting changes, branch-protection changes, merge, QA execution, OPS execution, or deployment, and they do not require a particular YAML structure, job name, cache implementation, concurrency syntax, or change-detection mechanism.
...
1.   
   **Validate comparability.** The two reports in a release-gating comparison MUST match every field required for the applicable repeatability or regression mode above and use an isolated or dedicated runner with no intentionally concurrent build, test, or benchmark workload. A missing or impermissibly mismatched field is `FAIL`, not a waivable review note. Timestamps and run identifiers may appear as provenance but are excluded from functional equality and comparator calculations.  
2. **Capture exact sample posture.** Require the exact sample count, zero timeouts, and zero missing or replacement samples. Capture metrics and byte-level assertions for AB↔BA, two-run identity, Reader↔CLI parity, and the other correctness predicates above.  
3. **Summarize.** Produce a bench report artifact (titles and paths only) with histograms, percentiles, counters, every decisive comparator input, calculated delta, limit, per-check result, and overall result. No raw payloads or secrets.
...
Evidence and index shapes and the merge-gating sentinel: **HDE-Schemas & Artifacts**. Process and PR flow: **Change Process Guide**. Provider-specific dashboard and alert bindings: **Glow Infrastructure**. Transport bytes and success matrices: §10 and Appendix A of this document.
...
1. **Not a PR.** Ops tasks are not PR work. Any implementation or remediation document that includes Ops work MUST separate Ops tasks from repository-local DEV work and label the Ops items as Product Owner-authorized execution, IA-guided.  
2. **Ops task record (required fields).** Every Ops task record MUST include: Task ID, Owner (PO), Facilitator (IA), target system/service (name only, no secrets), intent/end state, constraints/safety rails, success criteria (observable outcomes), evidence to capture (artifact paths), rollback intent, and a secret-handling note (no plaintext secrets in docs or evidence).  
...
1. **No governance drift.** Ops tasks MUST NOT create new acceptance tokens or redefine acceptance semantics. If an Ops task affects acceptance, it MUST map to existing governance-defined acceptance posture and be proven via evidence artifacts.  
2. **Mechanics and work-register accountability.** Any Ops task included in an Epic MUST be represented as a component in the **HDE Mechanics Guide** and have a related task or subtask in the **HDE Build Checklist**. Any Ops task included in a CRD MUST be recorded under its CRD identity in **HDE CRD Records**. A CRD Ops task maps to the **HDE Mechanics Guide** only when it changes mechanics within that document’s authority.

**Epic and CRD change lanes.**

PF04 recognizes two governed top-level change containers:

* **Epic lane.** Product-development work originating in or explicitly mapped to the **HDE Build Checklist** uses Epic identity and PF09 accountability.  
* **CRD lane.** An observed feature, defect, operational need, governance need, or other system change originating outside PF09 uses CRD identity and **HDE CRD Records** accountability. A CRD may contain one or more PRs, Ops tasks, or both.  
* **Common process.** The **Change Process Guide** governs the common lifecycle and routes each lane to its controlling work register. The distinction in origin, identity, and work register does not reduce development, QA, evidence, acceptance, or closure rigor. Work moves or maps between the two registers only through an explicit authorized decision.  
* **Approval and execution authority.** Epic and CRD implementation may begin only after the applicable plan receives the approval required by current governance. Repository, direct-main, and Ops execution then follow their distinct current authorization boundaries; plan approval does not erase those boundaries.  
* **Canon changes.** A CRD may change canon only through an accompanying approved ADR. The ADR must identify the affected canon and drainage consequences; material CRD activity and decisions must be recorded in **HDE Build Notes** before permanent drainage. A CRD Plan alone does not authorize canon supersession.  
* **Assurance and claim separation.** Both lanes use the full applicable **Glow QA Guide** process and the exact-source acceptance model in §2.0 and §9.7. Implementation, repository adoption, CI, QA, Ops, acceptance, PF09 or PF30 record status, canon drainage, closure, deployment, and production readiness remain separate claims. A verdict or proof from an earlier candidate transfers only after applicable rerun or explicit scope-equivalence proof.
...
* **Acceptance impact:** which governed acceptance criteria or decisions are affected, using their exact PF, ADR, epic, CRD, task, test, schema, or artifact identifiers; list legacy token cross-references only when they are actually used.  
...
> Draft / binding surface: \\

> Stable record surface: \\
...
These rails apply equally to the Epic/PF09 and CRD/PF30 lanes, including implementation plans, Epic Plans, CRD Plans, QA plans, Epic or CRD records, acceptance artifacts, and closeout records that define acceptance criteria, make acceptance or completion claims, or bind governed evidence.
...
When PF09 or PF30 scope is recorded, the record MUST represent every applicable in-scope work item truthfully, including unresolved or incomplete items: PF09 tasks or subtasks in the Epic lane and PF30 CRD, PR, or Ops units in the CRD lane. A claim that a criterion is implemented, covered, satisfied, or accepted is valid only when the governed evidence exists and the linked check or decision supports that exact claim.
...
Plans and proofs MUST use stable source identifiers already owned by the applicable PF, ADR, epic, CRD, task, acceptance criterion, test, schema, artifact, PR, commit, or workflow. They MUST NOT invent local aliases that obscure the governing source.
...
* silent omission of unresolved assigned PF09 tasks or subtasks or unresolved assigned PF30 CRD, PR, or Ops units; and  
...
If the Product Owner or Governance waives or narrows PF23 for a particular plan, reviewers MUST record the local scope directive and state that PF04 exact-source identity, evidence integrity, QA, safety, Epic and CRD scope, and claim-separation rails remain fully in force.
...
Before asserting that no stable identifier exists for a requirement or QA behavior, a reviewer MUST recheck the applicable PF, ADR, epic, CRD, task, acceptance criterion, test, schema, artifact, PR, workflow, and current scoped approval records.
...
* **Plans are pointers.** Epic Plans and CRD Plans are pointer maps, not full SOPs.  
* **Identifier and evidence proportionality.** Plans SHOULD use existing PF, ADR, epic, CRD, task, acceptance-criterion, test, schema, artifact, PR, commit, and workflow identities. They MUST NOT introduce a token or equivalent cross-reference “just in case.”  
...
* **Implementation planning must not require extensive QA evidence.** Epic and CRD Implementation Plans and Implementation Guides MAY state QA objectives and closeout proof obligations but MUST NOT embed a full QA runbook or require QA evidence production as implementation work.  
...
* **Truth/proof blockers only.** Valid approval blockers remain limited to material truth, authority, scope, evidence, safety, phase, acceptance, public/private boundary, OPS/QA/PR category separation, applicable PF09 or PF30 accountability mapping, source-of-truth, token, canon-conflict, required-proof, or evidence-identity defects. Missing proof obligation, missing applicable in-scope work-register mapping, unverified acceptance-token claim, unauthorized scope expansion, unauthorized public Reader expansion, live-provider or external-action requirement under closed rails, secret exposure requirement, OPS work assigned to Codex, QA execution required before QA begins, PF23 treated as acceptance proof, PF20 treated as current authority, non-token proof labels claimed as acceptance tokens, or unclear PASS/FAIL posture remain valid blockers when proven. They are not syntax issues.  
* **Work-register accountability for task-like future work.** Backlog, future, deferred, optional, follow-up, remediation, runtime-gap, adapter-gap, vendor-gap, QA-discovered, OPS-discovered, evidence-gap, build-improvement, and later-work labels are scheduling or disposition labels only. They do not remove PF09 accountability in the Epic lane or PF30 accountability in the CRD lane when the item is task-like and affects implementation, QA, OPS, runtime behavior, evidence behavior, vendor behavior, architecture behavior, or product behavior.  
* **Required task-like item classification.** A plan, remediation guide, QA-readiness review, retrospective, closeout review, or future-work section that contains a task-like item MUST first identify its governed lane. Epic/PF09 work must map to an exact phased PF09 task or subtask or be identified as a PF09 gap. CRD/PF30 work must map to an exact CRD ID and controlling PF30 record or be identified as awaiting required CRD/PF30 registration and approval. Other items must be classified as outside both governed work lanes or as documentation/status drainage only. Work MUST NOT move or map between PF09 and PF30 without an explicit authorized decision. If a relevant PF09 parent and subtask both exist, subtask-level mapping is required; parent-task-only mapping is insufficient.  
* **Token and acceptance posture do not substitute for work-register mapping.** Acceptance-token posture, evidence availability, QA PASS, OPS evidence, PF10 supportability notes, board status, closeout recommendations, or documentation-drainage status MUST NOT substitute for exact PF09 or PF30 mapping where that accountability is required. A reviewer may block an artifact that creates unaccounted task-like backlog when the item affects implementation, QA, OPS, runtime, evidence, vendor, architecture, or product behavior and lacks applicable PF09 or PF30 mapping, an explicit unresolved registration or gap posture, an out-of-scope classification, or a documentation/status-drainage classification.  
...
* **Acceptance criteria must use stable source identifiers.** Acceptance claims in plans MUST identify their governing PF, ADR, epic, CRD, task, acceptance criterion, test, schema, or artifact source. Freeform acceptance language alone is insufficient. A legacy token MAY be included as an optional bounded cross-reference, but token issuance or tokenization is not a prerequisite.

### **9.7.8 Evidence bundle cross-check for local-bundle deliverables**

When a deliverable claims a **local bundle** of governed artifacts under a specific directory (example: `artifacts/ops/internal_version/*`), the applicable Epic Plan or CRD Plan **MUST** explicitly state:
...
* For steps intended for external AI review, Deliverables lists must still respect any existing evidence batching constraints defined in the QA process and evidence policy (titles-only in **Glow QA Guide** and **Change Process Guide**).  
...
* **ETag: "\\"** — identity over the **final LF-terminated canonical JSON** body (pre-compression); encoding-invariant.  
* **Vary: Authorization, Accept-Encoding** — required; additional Vary members allowed.  
* **Content-Length: \\** — required; equals the identity 200 body length (final LF-terminated canonical JSON; pre-compression); encoding-invariant.  
...
2. **birthdate** — English month abbreviations `Jan..Dec`; day zero-padded (`01..31`); four-digit year. Examples: `03-Jan-1980`, `21-Sep-1995`.  
3. **birthtime** — 24-hour `HH:MM`; zero-padded hour and minute (`00..23`, `00..59`). Examples: `00:00`, `09:30`, `23:59`.  
4. **location** — ASCII English `"City, Country"`; a **single** comma \+ space separator; no leading/trailing spaces. Examples: `Paris, France`, `New York, USA`.  
5. **Excluded legacy field** — the record excluded `tz` because the legacy contract derived timezone from location through geocoding.
...
* **Prepare fixtures & env.** Fixed fixture set, pinned env; do not vary counts/data between cuts.  
* **Warm-up then measure.** Discard warm-up; collect metrics during a fixed measurement window.  
* **Collect checks.**  
...
1. **Capture histograms/counters.** Engine/presenter/reader latency buckets; outcome counters with bounded labels.  
2. **Write artifacts (titles/paths only).** Bench report (percentiles \+ histogram snapshot), parity/identity logs, A7 header snapshots.  
   **C.4 SLO verification (binary)**  
3. **Inputs:** bench report for current cut; pinned SLO targets (§6.2).  
4. **Pass when:** (1) p95/p99 thresholds met by profile/metric; (2) A3/A4 pass in the same runs; (3) A7 spot checks pass; (4) rails posture honored.  
5. **Fail posture:** stop canary or rollback via pointer-flip (§5.2); open investigation.  
   **C.5 Reporting & evidence (titles-only)**  
6. **Bench report:** `artifacts/bench/bench_report_{release_id}.json`  
7. **Parity/identity logs:** `artifacts/bench/parity_identity_{release_id}.log`  
8. **Transport snapshots:** `artifacts/bench/transport_headers_{release_id}/<TRANSPORT_SNAPSHOT_FILE>`  
9. **Indexing:** update **Appendix D — Evidence Index** in the same change.  
   **C.6 CI hooks (titles-only)**  
10. **Jobs:** `ci/jobs/bench_math_transport.yml` (rails closed) `ci/jobs/bench_vendor_open.yml` (rails open; policy pinned) `ci/jobs/slo_verify.yml` (compare report vs targets)  
11. **Guards:** enforce keys-only logging; fail on payload/body/header-value patterns; assert one LF and canonical encoding on emitted JSON.  
    **C.7 Ops notes**  
12. **No schema drift.** Bench harness must not alter public bytes or schemas.  
13. **Keys-only logs.** Never log payloads or header values; secrets always redacted (see §7.1, §8.2).  
14. **Pointer discipline.** Promotions/rollbacks are pointer flips to `release_id`; record outcomes in the Change Log and update Appendix D.
