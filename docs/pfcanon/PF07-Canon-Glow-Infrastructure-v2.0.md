...
**HDAPI v2 conformance candidate evidence roots (names-only; pending PF12 binding).** PF07 records the following HDAPI v2 vendor-contract candidate path family as infrastructure path inventory only. These files are not acceptance-binding until the owning Schemas & Artifacts home catalogs them and the Human Evidence Index and Machine Mirror bind them.
