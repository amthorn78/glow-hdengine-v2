...
**Evidence parity (same‑PR).** Every narrative change **must** update, **in the same PR**: the human Evidence Index `docs/evidence/INDEX.json` \+ hash sentinel and the machine mirror `artifacts/evidence_index.jsonl`. The mirror is **records‑only canonical JSONL** (UTF‑8, sorted keys, compact, **one trailing LF**), **rejects unknown keys**, uses **ASCII field order** and **sort‑before‑write**, is a **single mirror file**, and each record includes a `proof_anchor` to a path-proof stored alongside the artifact. Narrative evidence artifacts may live only under governed paths (`artifacts/**`, `docs/**`, or `audit/**`) and must remain bound through PF12’s Human Evidence Index, Machine Evidence Mirror, and path-proof discipline; no path becomes authoritative by directory alone. All canonicalization and comparisons run with **`LC_ALL=C, LANG=C, TZ=UTC`**. *(Index/mirror schema, governed roots, and path-proof discipline: PF12; PR-first workflow: PF06 — titles-only.)*
...
**Missing-key sentinel (fail-closed).** Missing category, band, or perspective coverage MUST resolve to `missing_narrative_key` for both `personal_key` and `shared_key` in router evidence. This sentinel is not a fallback key and MUST NOT select alternate prose. If the selected key is `missing_narrative_key`, the composer treats it as a validation conflict and suppresses rather than relaxing lookup, swapping templates, or emitting partial text.
...
**Router coverage artifact (titles-only).**  
 The router coverage artifact is a keys-only, no-prose proof locked to the supported narratives router matrix:  
 `audit/gates/narratives/keys_10x4.table.json`.

The key-table verifies:

* 10-category roster and 40-row key-table expectation.  
* Supported category/band/perspective cases.  
* `personal_key` and `shared_key` rows where coverage exists.  
* `missing_narrative_key` for unsupported or missing cases, without fallback prose.

**Router parity artifacts (titles-only).**

* `artifacts/narratives/router/parity_abba.log` — AB↔BA and two-run identity log for router outputs (keys-only, no prose).  
* `artifacts/narratives/router/cli_http_parity.log` — CLI=HTTP parity compare for router responses where parity is defined.

**Acceptance posture (names-only).**

* Router key-table canonical JSON coverage is claimed with `JSON_CANONICAL_CHECK_OK` for the HDE-EPIC032 PR-01 posture.  
* Router parity rows may claim `CLI_READER_PARITY_OK`, `TWO_RUN_IDENTITY_OK`, and `COMPOSITE_ABBA_IDENTITY_OK` where those rows prove the corresponding behavior.  
* `NARR_REGISTRY_CLOSURE_OK` MUST NOT be inferred from the key-table artifact alone. It is a registry-closure claim only when the governing acceptance roster explicitly admits that token for the slice.  
* Evidence/index/mirror/path-proof tokens are routed by title to PF12, PF09, and PF04; this section names only the narrative-router artifact posture.

**Routing (titles-only).**  
 Evidence listing, indexing, and path-proof details live in PF12; gates and task status live in PF09; token admission and semantics live in PF04.

## **4.5 Narrative registry diff and pack identity evidence \[Canon\]**

**Purpose.**  
 Narrative registry diff and pack identity evidence prove that a narratives pack change is keys-only, deterministic, manifest-bound, and indexable without exposing narrative prose or inventing acceptance tokens. This section names the narrative evidence posture only. Generator implementation, pipeline mechanics, evidence schemas, and PF09 task status live in their owning documents by title.

**Registry diff artifact (titles-only).**

* `audit/gates/narratives/registry.diff.json` is the canonical, keys-only registry diff artifact for narrative registry changes.  
...
* The artifact MUST use canonical JSON and must be suitable for governed evidence indexing.  
* If no prior baseline exists, the artifact may truthfully record a current-manifest-verified posture, but it MUST still fail closed on malformed, unsupported, or incomplete manifest or key state.

**Pack identity artifact (titles-only).**

* `audit/gates/narratives/pack_identity.txt` records pack identity evidence for the narrative pack under review.  
* Pack identity evidence MUST bind `pack_sha` to the SHA-256 of canonical manifest bytes and include enough identity material to prove the manifest path, canonical size, and two-run stability.  
* Pack identity evidence MUST remain keys-only and provenance-only; it MUST NOT contain narrative prose.

**Validation posture.**

Narrative registry diff evidence is valid only when validation fails closed for:

* missing or unexpected manifest paths,  
* unsupported category values,  
* unsupported band values,  
* unsupported perspective values,  
* unsupported slot values,  
* duplicate category-band-perspective-slot tuples,  
* duplicate keys where uniqueness is required,  
* missing supported category-band-perspective-slot tuples, and  
* partial slot sets for supported groups.

Missing, unsupported, duplicate, or partial registry state MUST NOT be normalized into fallback prose. The diff evidence must preserve the fail-closed posture already defined for `missing_narrative_key` and suppression.

**Doc-Delta and evidence binding.**

Narrative registry diff and pack identity changes MUST carry a Doc-Delta posture artifact when required by the epic or PR slice. For HDE-EPIC032 PR-02, the governed Doc-Delta posture artifact is `audit/docdeltas/hde-epic032_doc_deltas.md`.

Registry diff, pack identity, and Doc-Delta posture artifacts MUST be bound through the same governed evidence discipline as other narrative evidence:

* human Evidence Index update,  
* hash sentinel update,  
* Machine Evidence Mirror update,  
* path-proof binding, and  
* final-LF and canonical JSON checks where applicable.

**Pipeline ordering (titles-only).**

For PRs that claim narrative registry diff or pack identity evidence, generation and check of narrative registry diff evidence MUST run before evidence index update and evidence index check. PF17 records the narrative evidence dependency only; the concrete sanity pipeline, test names, and CI wiring live in HDE-Mechanics Guide, HDE-Build Checklist, and HDE-Schemas & Artifacts by title.

**Acceptance posture (names-only).**

Narrative registry diff and pack identity evidence may support only the token names admitted for the governing slice. In HDE-EPIC032 PR-02 posture:

* registry diff canonical JSON may support `JSON_CANONICAL_CHECK_OK`,  
* pack identity two-run stability may support `TWO_RUN_IDENTITY_OK`,  
* Doc-Delta posture may support `DOC_DELTA_PRESENT_OK`, and  
* evidence/index/mirror/path-proof tokens route by title to HDE-Governance, HDE-Schemas & Artifacts, and HDE-Build Checklist.

`NARR_REGISTRY_CLOSURE_OK` MUST NOT be claimed from narrative registry diff, pack identity, Doc-Delta, index, mirror, or path-proof evidence unless the governing acceptance roster explicitly admits that token for the slice.

**Routing (titles-only).**

* PF12 owns schemas, canonical JSON, manifest listing, Evidence Index, Machine Evidence Mirror, hash sentinels, path-proof rules, and artifact catalog entries.  
* PF14 owns generator mechanics, sanity pipeline mechanics, and tests.  
* PF09 owns build-checklist task status and gate wiring.  
* PF04 owns token admission and token semantics.  
* PF06 owns same-PR evidence parity and Doc-Delta process discipline.
