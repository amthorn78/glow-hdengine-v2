...
* BodyGraph resolver and vendor ingest use ephemeral QA keys and vendor-backed behavior:
  ...
  * In prod pre-App:
    ...
    * For offline/local checks of resolver or ingest:
      ...
      *  `bg:resolve --source=vendor --dry-run` under open rails may be used only for the exact BodyGraph resolver or ingest-path behavior it actually exercises. In the current HDAPI v2 posture, this path is a legacy BodyGraph ingest-path observation when the configured base URL carries the v2 prefix. It MUST NOT be treated as the canonical v2 chart/geokey validation path, a v2 ChartResult-to-BodyGraph adapter proof, or compatibility proof.  
      *  To prove v2 chart/geokey header posture, QA or OPS evidence must use an explicitly scoped v2 chart route observation, such as \`charts\` or \`charts/simple\`, that records \`Authorization: Bearer \<redacted\>\`, \`HD-Geocode-Key: \<redacted\>\` when geocode is required, and legacy \`HD-Api-Key\` absent on the v2 chart path. That evidence remains bounded to the exercised route and does not by itself prove full HumanDesignAPI v2 runtime conformance.  
...
* HDAPI v2 response-normalization proof MUST either show that the selected v2 vendor payload can feed the existing HDE BodyGraph, cache, compatibility, sampler, or admin paths without changing public Reader bytes or leaking admin-only data, or else record the exact adapter/schema gap. If v2 response data cannot truthfully map into the existing model, QA MUST record the gap plainly rather than smoothing it over by inference.  
* A recorded ChartResult or ChartSimpleResult adapter/schema gap may be acceptable for a scoped evidence-slice whose purpose is gap recording. It is not acceptable as future proof that v2 chart data feeds the existing BodyGraph cache, person/bodygraph compute input, compatibility path, sampler path, or app integration. Future runtime or production claims require a bounded adapter/schema proof or implementation that identifies the vendor payload family used, required vendor fields, populated internal BodyGraph/person/cache fields, intentionally absent fields, any legacy fallback, raw-payload retention or exclusion posture, and whether the adapter is sufficient for HD Engine compute.  
* A simple chart smoke may prove a narrow vendor route, authentication, geocode-key, or provider-availability observation. It MUST NOT be accepted as proof of full BodyGraph-detail resolution unless a future adapter/schema proof shows that the simple chart response contains every field required by the existing BodyGraph/person/cache contract.  
* Future QA for `bg:resolve --source vendor` MUST verify the actual selected BodyGraph-resolution policy, not a nearby smoke route. Acceptable future postures are v2 chart-backed BodyGraph resolution, explicit legacy BodyGraph fallback, explicitly approved dual-route behavior, or no runtime vendor-resolution claim. QA must treat accidental composition of a legacy BodyGraph resource against a configured v2 base as a route-policy gap unless vendor evidence and the owning PF homes explicitly support that combination.  
* QA must classify route-shape mismatch separately from provider unavailability. A 404 from a mismatched `bg:resolve` request shape is not proof that the vendor is unavailable when a scoped v2 chart route succeeds. A simple chart success may prove provider availability, v2 auth posture, geocode-key posture, and route-family availability for that route, but it does not prove full BodyGraph detail, cache compatibility, compute compatibility, or canonical BodyGraph-resolution behavior.  
