...
Ops tasks are not QA tasks. They are implementation work that changes runtime state outside the repo and is executed by the PO.
...
Implementation Plans must be concrete enough for CodEx and OPS separation. Formatting defects are blockers only when they create real CodEx or OPS ambiguity, source-authority ambiguity, evidence-trust defect, or execution-safety risk.

Plan-execution artifact boundary.

Plans, review prompts, redline prompts, Codex prompts, remediation plans, QA Plans, Live QA Plans, Implementation Plans, Epic Plans, and closure-review artifacts are planning and review artifacts, not execution artifacts.

They MUST NOT be blocked, rejected, returned for revision, or classified as REVISE AND RESUBMIT solely because a command, code snippet, heredoc, shell line, helper function, example invocation, indentation block, markdown-rendered string, escaped character, or similar example is not paste-ready, literal, syntactically exact, or executable as written.

This rule applies even when the syntax concern appears in raw source text and even when a reviewer believes the command would fail if pasted directly.

QA steps and plan commands do not need to include final runnable syntax when the proof target, scope boundary, rails posture, acceptance posture, public/private boundary, no-secret posture, no-new-token posture, no-new-scope posture, and expected evidence family or verdict posture remain clear.

In-flight command normalization is execution hygiene. During execution, the assigned operator, QA executor, Codex, Kronos, Product Owner, or implementation owner may normalize command syntax, indentation, heredoc form, shell syntax, helper-code formatting, interpreter invocation, or rendered escaping when the proof target, step identity, scope, rails, evidence intent, acceptance posture, and non-claim boundaries remain unchanged.

In-flight syntax normalization does not require plan rejection, a remediation guide, a PF10 addendum, or a QA Plan revision unless the underlying proof target, scope, source authority, safety posture, evidence identity, or acceptance posture changes.

A valid plan-approval blocker must identify material harm to truth, proof, scope, authority, safety, acceptance posture, phase fidelity, evidence identity, PF09 completion mapping, public/private boundary, OPS/QA/PR separation, canon conflict handling, required proof obligations, or closeout truth.

Reviewers MUST NOT disguise command exactness, paste-readiness, syntax, escaping, indentation, heredoc form, helper-code formatting, or formatting complaints as truth, proof, implementation, QA, evidence, token, path-proof, PF locator, readiness, or closure blockers.

Command or syntax concerns in planning artifacts may be recorded only as Non-issue, Note, In-flight normalization, Operator caution, Nit, Suggestion, or Caveat, unless a separate non-syntax defect remains after ignoring syntax and rendering artifacts.

Syntax concerns MUST NOT be classified as Blocker, Approval blocker, QA readiness blocker, Implementation readiness blocker, Closure blocker, FAIL\_BEHAVIOR, FAIL\_TOOLING, TOOLING\_BLOCKED, acceptance failure, path-proof failure, evidence failure, token failure, PF locator failure, or command validity failure requiring plan revision.
...
If a reviewer returns REVISE AND RESUBMIT, the review MUST state the non-syntax truth, proof, scope, authority, safety, acceptance, phase, or evidence-identity reason.
...
* Rendered escape characters that plausibly arise from AI processing, markdown rendering, display-layer normalization, retrieval formatting, reviewer quoting, or chat transcript display are not source truth by themselves.  
  ...
  * A valid escape-character blocker MUST identify the raw source file or governed artifact inspected, the exact read-only command or source-view method used, the raw line showing the unwanted character, why the character changes executable, governed, canonical, or semantic identity, and why the finding is not merely assistant, markdown, review, transcript, or display rendering.  
  * Without all required proof fields, the blocker is invalid and MUST be withdrawn or reclassified as display-layer artifact.  
  * Redline authors MUST NOT draft corrective redlines solely to remove display-layer escape artifacts. A redline to remove an escape character is allowed only when the raw source or raw target document actually contains the unwanted character and that character changes identity or meaning.  
  * Codex prompt authors MUST NOT ask Codex to create, rename, remediate, or fix escaped paths, filenames, commands, or artifact strings derived from rendered assistant output. Codex prompt text MUST preserve the intended unescaped identity unless raw source proof establishes a real escaped-source defect and the approved plan directs the correction.  
