...
**DB bridge/provider parity proof-label posture.** `DB_PROVIDER_PARITY_OK`, `DB_BRIDGE_CAPS_OK`, and `DB_BRIDGE_FALLBACK_OK` are non-token proof labels unless and until those exact names are explicitly admitted into this Token Registry or minted by PF10 pending drainage.

* Implementation plans, PR summaries, OPS evidence, QA logs, acceptance maps, token/evidence matrices, and closeout artifacts MUST NOT claim `DB_PROVIDER_PARITY_OK`, `DB_BRIDGE_CAPS_OK`, or `DB_BRIDGE_FALLBACK_OK` as satisfied acceptance tokens unless those exact names are registered here or minted by PF10 pending drainage.  
...
* If close-stage review determines that provider parity, bridge capability, or non-dev bridge fallback must become gated acceptance predicates, the token names and semantics MUST be admitted through Governance before any acceptance artifact claims them.  
* Provider-parity proof labels MUST distinguish deterministic comparison rows, unavailable rows, skipped rows, missing rows, and errored rows. A provider-parity artifact MUST NOT report `PASS`, equivalent, or supportable parity if the direct-provider rows or bridge-provider rows required for the claimed corpus are missing, skipped, unavailable, errored, or otherwise not compared. Truthfully recording unavailable parity under closed rails is allowed, but it is not parity PASS and not a status-closure claim unless a registered token or PF10 addendum explicitly says so.  
* **Provider-parity OPS closure-evidence posture.** A provider-parity OPS evidence task MAY be accepted as closed OPS evidence only when the governed closure packet proves all of the following without contradiction: the active parity corpus is explicit, each active row is non-skipped, direct-provider and bridge-provider rows required for the claimed corpus are available, every active row records `parity=match`, the bridge consistency result is `PASS`, and the closure decision or equivalent governed artifact records the closed provider-parity posture.  
* **Provider-parity OPS evidence bundle.** The governed OPS evidence bundle for provider-parity closure MUST preserve the command transcript, working directory when relevant, stdout posture, stderr posture, exit code, target environment, presence-only DB environment posture, parity proof, bridge consistency result, scope rationale, non-claims, final report or summary, checksum ledger, and any closure-decision artifact required by the approved task.  
* **Provider-parity OPS non-claim boundary.** Provider-parity OPS closure evidence is OPS evidence only unless a later governed review or registered token explicitly says otherwise. It MUST NOT by itself claim QA PASS, PF09 status movement, epic closure, acceptance-token satisfaction for DB proof labels, or PF-canon drainage. Evidence may support later review, QA, closeout, or checklist drainage only when those later artifacts state the supportable posture truthfully.  
* **Combined-evidence supportability for DB bridge/provider parity scope.** A later governed review, PF10 addendum, closeout record, or checklist-drain supportability decision MAY rely on a combined proof chain across implementation evidence, OPS parity closure evidence, and later evidence-coherence binding when all of the following are true: each prior slice preserved its own non-claim boundaries truthfully, the combined record names the slice roles, the governed evidence families are current and coherent, and the combined record states the exact later-drain posture being supported.  
* Earlier slice-local no-status-move language MUST NOT be read as prohibiting a later combined-evidence supportability decision when the earlier non-move was true only for that bounded slice and the later combined record preserves that history plainly.  
...
* **DB\_CONN\_ENV\_OK** — DB connections in each environment use the governed source: `DATABASE_URL` for primary DB, `DB_BRIDGE_URL` only where bridge use is explicitly allowed, and mocks/fixtures only in dev/test. Prod public traffic must not use `DB_BRIDGE_URL` for Reader/Aux routes. Connection failures are typed and recorded as evidence (artifacts and schemas live in HDE-Schemas and Artifacts and Glow QA Guide); ad-hoc connection logic is forbidden.  
  * **Production-like DB bridge guard posture.** When DB connection policy or governed evidence uses `APP_ENV` to decide whether bridge fallback is allowed, `prod`, `production`, and `live` MUST be treated as production-like for bridge-guard purposes. Production-like environments MUST select `DATABASE_URL` first and MUST NOT fall back to `DB_BRIDGE_URL` unless the approved production bridge override is explicitly set. This is a hardening posture and MUST NOT be described as a separate `live` runtime policy or permissive runtime mode.  
  * **Non-dev total-failure evidence posture.** When non-dev DB connection failure behavior is used as governed evidence for `DB_CONN_ENV_OK`, the evidence MUST use the non-dev environment posture being claimed, record the real observed provider-attempt order, avoid proactive probes unless explicitly approved by the governing plan, preserve secret-free and numeric-free public failure posture, and surface the expected typed failure as `BridgeUnavailable` with the governed missing-bridge reason when that is the claimed failure path. Synthetic unexpected outputs, hardcoded selection order, or serialized success from an unexpected path MUST fail closed and MUST NOT be accepted as proo  
...
* Close-pack baseline artifacts (non-token; deterministic path-of-record). Close-pack presence is a baseline closure artifact requirement, not an acceptance token by default.  
  ...
  * **Close report minimal content (normative).** `audit/EPIC-###_close_report.md` MUST be a human-readable closure summary and MUST include:  
    ...
    * **Repo-supported completion summary (required when closeout is recommended from repo evidence)**.The closeout record MUST state whether the completion summary is limited to repo-supported completion and MUST distinguish repo-supported completion from canon drainage and from formal close-pack completion.  
    ...
    * **No over-claim rule.** The closeout record MUST NOT imply that canon drainage is complete, that formal close-pack completion is complete, or that merge provenance is established unless those claims are directly evidenced by governed artifacts in the reviewed source set.  
...
**HDAPI v2 open-rails vendor proof posture.** A controlled HDAPI v2 open-rails smoke is an Ops evidence task when it requires secrets or privileged runtime posture. It MUST be PO-only, IA-guided, secret-safe, and stored under a lowercase audit ops path. Automated agents MUST NOT execute the vendor call, claim completion, or simulate external state.
...
* **Live QA Plan review burden.** A reviewer who blocks a Live QA Plan MUST state the operational harm. Invalid blocker framing includes objections that only say the command is not the exact preferred spelling, the command is wrapped differently, helper code is not formatted cleanly, markdown rendering shows escape characters, step-log headers are not template-perfect, a code block is used, or source quote formatting is imperfect.  
