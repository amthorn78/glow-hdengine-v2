...
**Last Update Gate:** 080826 Refresh 1
...
- Governed JSON artifacts referenced from PF03 follow HDE Schemas and Artifacts for canonical JSON requirements.  
...
OPS tasks posture — MUST. If a work item requires privileged access outside the repository (service config, secrets/env var changes, deploy/runtime settings, infra consoles, privileged DB operations), it is an OPS task. OPS tasks MUST be authorized by the Product Owner. The PO may execute an authorized task personally or explicitly delegate execution to an automated session agent. The delegated agent MAY perform the authorized operation on the PO's behalf and MUST follow the same scope, safety, evidence, redaction, and completion-claim controls that bind a human operator. No agent or human operator may claim OPS completion without the required repo-stored, secret-free evidence, and no agent may simulate external state changes. Any doc that includes OPS tasks MUST separate them from PR work and MUST specify evidence capture as repo-stored, secret-free artifacts under lowercase audit paths (for example, `audit/`).
...
* **Syntax-origin command defects remain non-blocking — MUST.** A Live QA Plan, QA Plan, remediation plan, or plan-review artifact MUST NOT be blocked when the causal source of a command-note defect is syntax, quoting, escaping, shell grammar, tokenization, command-wrapper form, heredoc form, helper-code form, variable interpolation, indentation, whitespace, Markdown rendering, code-block formatting, backslash insertion, copy-and-paste corruption, interpreter invocation form, or literal command presentation. The defect remains syntax-origin even when the literal text fails to parse or execute, executes text different from what was intended, loses quotes, delimiters, or string literals, changes apparent command identity, invokes an incorrect apparent command form, cannot reach the intended behavior, cannot be pasted and run without correction, or would produce a different result because of the malformed representation. Raw-source confirmation proves where the syntax issue appears; it does not make the issue substantive.  
* **Syntax-normalized command review — MUST.** Plan reviewers MUST identify the intended objective, operation, inputs, outputs, rails posture, evidence target, and PASS or FAIL predicate; assume a syntactically valid representation that preserves those semantics; determine whether the alleged defect remains after normalization; and exclude the issue from the review decision when normalization removes it. If the reviewer cannot prove that an issue survives faithful syntax normalization, the issue is syntax-origin and non-blocking. A literal paste-and-run test is not a plan-approval test.  
* **Correction boundaries — MUST.** Syntax normalization MUST NOT change scope, objectives, repo loci, dependencies, semantic inputs, required outputs, authorization, rails posture, evidence obligations, acceptance conditions, or PASS and FAIL predicates. It MUST NOT invent a new command source, route, artifact family, or acceptance predicate.  
* **Correction evidence — MUST.** When a QA executor corrects a plan command syntax defect during execution, the governed step evidence MUST record the exact command actually executed, the command provenance, the reason for correction, the exit code, the captured output, the produced evidence artifacts, and the final PASS, FAIL, or TOOLING classification.  
* **Independent non-syntax command blockers remain blockers — MUST.** A substantive command finding may block only when it identifies a requirement, contract, dependency, scope boundary, evidence obligation, authorization rule, or execution-context condition independent of malformed syntax; states the syntax-normalized intended operation; proves that the defect remains after faithful syntax normalization; and relies on evidence other than the malformed command representation. A mixed finding MUST be separated, the syntax component discarded, and only the independently proven non-syntax component evaluated.  
...
* **Plans are not execution artifacts — MUST.** QA Plans, Live QA Plans and their revisions or resubmissions, Epic Plans, Implementation Plans, remediation plans, plan redlines, implementation-readiness reviews, QA closeout reviews, epic closure reviews, review prompts, Codex prompts, and any other review whose decision depends on the adequacy of a planning artifact MUST NOT be blocked, rejected, returned for revision, or classified as REVISE AND RESUBMIT solely because a command, code snippet, heredoc, shell line, helper function, example invocation, indentation block, markdown-rendered string, escaped character, or helper-code block is not paste-ready, literal, syntactically exact, or executable as written.  
* **Causal-source classification controls review — MUST.** The causal source of a defect controls its classification; its downstream effect does not. A defect caused by syntax, quoting, escaping, shell grammar, tokenization, command-wrapper form, heredoc form, helper-code form, variable interpolation, indentation, whitespace, Markdown rendering, code-block formatting, backslash insertion, copy-and-paste corruption, interpreter invocation form, or literal command presentation remains syntax-origin even when the literal text fails to parse or execute, executes text different from what was intended, loses quotes, delimiters, or string literals, changes apparent command identity, invokes an incorrect apparent command form, cannot reach the intended behavior, cannot be pasted and run without correction, or would produce a different result because of the malformed representation. Raw-source confirmation proves where the syntax issue appears; it does not make the issue substantive.  
* **Syntax-origin defects have no adverse review disposition — MUST NOT.** A syntax-origin defect MUST NOT be used as a Blocker, Caveat, required Nit, Expected fix, Needs revision determination, Section Checklist revision basis, Review Ledger finding, readiness or approval condition, reason for plan revision or resubmission, reason to withhold closure, or any other input that worsens or conditions the review decision. Reviewers MUST NOT relabel a syntax-origin defect as changed executable meaning, changed command identity, loss of runability or runnability, literal non-executability, mechanical invalidity, wrapper corruption, failure to reach behavior, proof-command failure, or source-byte confirmation.  
* **Syntax-normalized review method — MUST.** Reviewers MUST identify the intended objective, operation, inputs, outputs, rails posture, evidence target, and PASS or FAIL predicate; assume a syntactically valid representation that preserves those semantics; determine whether the alleged defect remains after normalization; and exclude the issue from the review decision when normalization removes it. If the reviewer cannot prove that an issue survives faithful syntax normalization, the issue is syntax-origin and non-blocking. A literal paste-and-run test is not a plan-approval test. Syntax normalization MUST NOT change scope, objectives, repo loci, dependencies, semantic inputs, required outputs, authorization, rails posture, evidence obligations, acceptance conditions, or PASS and FAIL predicates.  
* **Independent non-syntax blocker standard — MUST.** A substantive finding may affect a review decision only when it identifies a requirement, contract, dependency, scope boundary, evidence obligation, authorization rule, or execution-context condition independent of malformed syntax; states the syntax-normalized intended operation; proves that the defect remains after faithful syntax normalization; and relies on evidence other than the malformed command representation. A mixed finding MUST be separated, the syntax component discarded, and only the independently proven non-syntax component evaluated.  
...
* **Syntax concern severity vocabulary — MUST.** A concern that remains syntax-origin after faithful normalization may be classified only as Non-issue, Note, In-flight normalization, or Operator caution. Downstream effects of the malformed representation do not widen this vocabulary.  
...
* **Reviewer burden for revise-and-resubmit — MUST.** Any reviewer returning REVISE AND RESUBMIT must state the independently proven non-syntax truth or proof reason and show that it remains after faithful syntax normalization. If the reviewer cannot prove that survival, the issue is not a blocker.  
...
* **Operational blocker materiality test — MUST.** After faithful syntax normalization, a Live QA Plan approval blocker is valid only when an independently proven non-syntax issue affects safe execution, required QA step coverage, required deliverable existence, explicit PASS or FAIL verdictability, rails posture, secret handling, live-provider or external-action boundary, public or private surface boundary, token truth, acceptance overclaim, source authority, self-contained execution, evidence trust, proof target identity, repo-locus truth where the plan requires an existing locus, OPS, QA, or implementation category separation, phase scope, or closeout truth.  
* **Exact-command mismatch requires independent non-syntax harm — MUST.** A command mismatch may block only when an independently proven non-syntax defect remains after faithful syntax normalization and would run the wrong tool, prove the wrong target, open unsafe rails, expose secrets, mutate prohibited state, prevent the intended check from running, or create a false PASS or false FAIL with no safe fallback. Those downstream consequences do not make malformed syntax substantive by themselves.  
* **Equivalent safe commands are non-blocking execution notes — SHOULD.** If a different invocation can produce the same proof under safe rails and actual command capture is required, classify a syntax-origin difference as a Non-issue, Note, In-flight normalization, or Operator caution. Use Caveat only when an independently proven non-syntax risk remains after faithful normalization.  
...
* **Database-role planning categories must remain distinct — MUST.** Distinguish observed live role state, target-role absence, desired security posture, authorized mutable precursors, direct capture posture, provisioned bounded posture, and the separately governed complete role architecture. Repository names, fixtures, schema fields, acceptance predicates, PF wording, and planned architecture do not prove that a corresponding live role exists.  
* **One-attempt database authorization planning requires live proof — MUST.** Before binding a one-attempt database authorization, inspect the live role roster and effective privileges; establish whether an existing identity satisfies the exact operational predicate; obtain Product Owner authorization for any required mutable precursor; complete and verify that precursor before constructing authorization bytes; retain exact SQL or an authoritative equivalent when later claims depend on the complete grant model; preserve pre-state and post-state when claiming that existing roles remained unchanged; and exclude credential values from retained evidence.  
* **Missing required open-rails QA is a substantive coverage blocker — MUST.** Reviewers must block a QA Plan, Live QA Plan, QA-readiness review, or closeout-review posture when the work affects a user surface, production surface, CLI surface, vendor ingestion, vendor transport, persistence, runtime route, environment binding, secret binding, operational surface, or deployed behavior, and the plan or posture lacks both a bounded open-rails live QA step and a clear authorized exemption. The blocker must name the affected surface and state that closed-rails evidence does not satisfy the missing live proof.  
...
* **Audit provenance is valid planning context — MUST.** Plans, QA Guides, QA Plans, review artifacts, and retrospectives may reference audit provenance when it explains why work exists, what prior review observed, what risk or ambiguity was surfaced, what repo area should be inspected, what PF-canon or PF09 mapping may need attention, why a future proof obligation exists, why a gap is carried forward, or why a workstream is planned.  
* **Audit provenance is not operative authority — MUST NOT.** Do not convert audit provenance into PR instructions, OPS instructions, step-by-step execution procedure, Codex command source, acceptance authority, token authority, QA PASS proof, OPS completion proof, PF09 Done proof, closeout proof, current repo truth without repo validation, invented path or module proof, required deliverable authority, privileged live-action authority, or secret/external-state authority.  
* **Audit context must be converted before execution — MUST.** Audit provenance may appear in plan context, task rationale, Tracked Issues, ADR stubs, evidence rationale, and review history. Operative PR and OPS instruction blocks must convert that context into neutral work language grounded in PF10, PF-canon, PF09 mapping, approved plan scope, and current repo validation.  
* **Audit-provenance review posture — MUST.** Reviewers must not block a plan solely because it includes audit provenance as context. A blocker is valid only when the plan uses the audit as execution authority or proof authority, or when the audit claim is presented as current repo truth without current repo validation.  
...
* Execution wrapper must be fixed before execution — MUST. The contract MUST define the execution wrapper or execution capture method before the authorized executor runs the smoke. The wrapper must capture stdout, stderr, exit code, and deterministic capture posture where applicable.  
...
* PO-authorized external execution — MUST. Any controlled external or vendor-backed smoke that requires privileged external access, credentials, open rails, or vendor calls MUST be PO-authorized and IA-guided. The PO may execute the call personally or explicitly delegate execution to an automated session agent. A delegated agent MUST follow the same scope, safety, evidence, redaction, and completion-claim controls that bind a human operator. No executor may claim completion without the required secret-free evidence or simulate external state changes.  
...
OPS linkage — MUST. Any step labeled OPS MUST follow the OPS posture in §4 and the OPS task record template in §15.5 (PO-authorized execution, IA-guided, not PR work, secret-free evidence, lowercase audit paths).
...
OPS tasks — PO-authorized procedures only: `OPS-01`, `OPS-02`, \[LIST CONTINUES\]
