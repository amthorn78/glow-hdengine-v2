...
* Selects two registered narrative keys per category: `personal_key` and `shared_key`. Perspective routing uses `shared`, `a_to_b`, or `b_to_a`.
...
* Give explanations, not only scores: two short narrative lines per category (one direction-aware Private line and one Shared line) chosen by rules (no HD jargon in user copy).  
...
* Drive narrative output (no copywriting).  
  * Produce narrative keys and selection signals for each category and perspective (`shared`, `a_to_b`, or `b_to_a`) using viewer intent and routing flags.  
...
* Adapter surface (HTTP only). Wires Reader routes and the ops endpoint; does not define public bytes (route by title to HDE-CLI-API-Vendor-Ref).  
* Mechanics core. Pure orchestration for math, comparators, config loader, and public emitter wiring (route math to HDE-Math-Spec).  
* Schemas & validation. JSON-Schema validation for governed inputs; AJV (or equivalent) CI step that fails on schema drift (route schema/canonical rules to HDE-Schemas & Artifacts).  
* Evidence indices.  
  * Human index (titles/paths only) in this repo, updated in the same PR as artifacts.  
  * Machine JSONL mirror (records-only) owned by HDE-Schemas & Artifacts; CI enforces 1:1 parity and path-proofs for every record.  
...
* Human index. docs/evidence/INDEX.json — titles and paths only; no payload bytes. It must maintain 1:1 parity with the Machine Mirror; see §37.4 for the local registry and HDE-Schemas & Artifacts §8.3 for the owning schema.  
...
* Whenever any golden, snapshot, or script moves or changes, update the Human Evidence Index (docs/evidence/INDEX.json) and the machine mirror in the same commit/PR. CI fails on mismatch.
...
docs/evidence/INDEX.json and docs/evidence/INDEX.sha256 are governed artifacts and MUST each have an up-to-date \*.path\_proof.txt. In normal write mode, tools/evidence/update\_evidence\_index.py MUST refresh the path proofs for the Human Evidence Index and its hash sentinel. In `--check` mode, the tool MUST remain read-only and fail when the bytes that normal write mode would produce differ from the governed bytes on disk.
...
* emitting a bundle manifest (JSON/JSONL) that lists, for each member, at least: logical artifact\_key, sha256, size\_bytes, and any additional descriptors defined in HDE-Schemas & Artifacts.
...
* Run `python tools/evidence/update_evidence_index.py` to regenerate the Human Index, Machine Mirror, and path proofs for both sampler and Engine Core evidence so that `produced_at_utc` is coherent across each refreshed artifact, Mirror record, and proof transcript. Then run `python tools/evidence/update_evidence_index.py --check` as a read-only equivalence check.

Evidence Index and mirror  
Run `python tools/evidence/update_evidence_index.py` to refresh, then run `python tools/evidence/update_evidence_index.py --check` to validate without writing:
...
Public bytes: HDE-CLI-API-Vendor-Ref; Math & preimage: HDE-Math-Spec; Schemas, pack/manifest, canonical bytes, machine mirror: HDE-Schemas & Artifacts; Governance (A7, ops posture, Evidence Index policy): HDE-Governance
...
The concrete EPIC027 records-only path inventory is single-homed in §37.4. This section defines generic harness behavior and MUST NOT duplicate that registry.
...
Identity coupling. release\_id is taken from the freeze-pack manifest (PF12-Canon-HDE-Schemas-and-Artifacts §6). meta.invocation\_tag participates in the preimage (HDE-Math-Spec).
...
## **8.7 Evidence (records-only; path-agnostic; indexed via PF12-Canon-HDE-Schemas-and-Artifacts Machine Mirror)**

Evidence artifacts are indexed and mirrored according to PF12-Canon-HDE-Schemas-and-Artifacts (§8.3 / §8.6).
...
Machine Mirror record fields, semantics, and order are defined in PF12-Canon-HDE-Schemas-and-Artifacts §8.3.
...
This section governs mechanics only; all concrete bytes, tokens, and schemas are owned by the owning documents above.
...
List titles/paths in the Human Evidence Index (docs/evidence/INDEX.json) and mirror them 1:1 in artifacts/evidence\_index.jsonl. Machine Mirror record fields, semantics, and order are defined in PF12-Canon-HDE-Schemas-and-Artifacts §8.3. Update the Human Evidence Index and Machine Mirror in the same PR, with path-proofs.

Routing (titles-only):

* Transport matrices & A7 policy: PF05-Canon-HDE-CLI-API-Vendor-Ref / HDE-Governance  
...
Routing (titles-only). Policy lives in HDE-Governance §10.5; transport matrices for success routes live in PF05-Canon-HDE-CLI-API-Vendor-Ref (A7 not applicable here).
...
Strict schema. Requests MUST validate against the governing schema (titles-only to PF12-Canon-HDE-Schemas-and-Artifacts). Unknown or extra keys are rejected (typed error).
...
Routing only. Transport behavior is governed in HDE-Governance §10 and matrices live in PF05-Canon-HDE-CLI-API-Vendor-Ref. This guide does not restate headers or validator details.

Writers and errors:

* Cache-Control: no-store; no ETag.  
    
* Errors: Content-Type: application/json; charset=utf-8; typed, numeric-free error bodies (see PF05-Canon-HDE-CLI-API-Vendor-Ref error model).  
    
* Writers have no HEAD/304 semantics.

Success endpoints (A7 proofs):

* Proofs run only on a cataloged Endpoint Catalog (JSON success) route (see PF05-Canon-HDE-CLI-API-Vendor-Ref §5.6 / Appendix A).  
...
* No locale/I/O: static/grep checks show no file/env/time use in writer modules.
  ...
  ## **10.6 Evidence (records-only; path-agnostic; indexed via PF12-Canon-HDE-Schemas-and-Artifacts machine mirror)**
...
Machine Mirror record fields, semantics, and order are defined in PF12-Canon-HDE-Schemas-and-Artifacts §8.3. Update the Human Evidence Index and Machine Mirror in the same change; CI enforces human↔machine parity and path-proofs.

Routing (titles-only):

* Transport rules & error headers: HDE-Governance (A7; writers/errors).  
    
* Error shapes & public bytes: PF05-Canon-HDE-CLI-API-Vendor-Ref.  
    
* Schemas & canonical JSON: PF12-Canon-HDE-Schemas-and-Artifacts.
...
* Execute the shared evidence synchronization and validation workflow defined in §37.18.1.

# 11\) Input Normalization & Validation Layer

Scope. Normalize IDs and validate payloads against schemas (titles-only to PF12-Canon-HDE-Schemas-and-Artifacts). Mechanics wires the checks; it does not restate schemas here.
...
* If input aliases are explicitly enabled, they must normalize via the declared alias ledgers (titles-only to PF12-Canon-HDE-Schemas-and-Artifacts A1/A4/A5). Outputs remain canonical.
...
* Schema pass. Payloads validate against their owning schemas (titles-only to PF12-Canon-HDE-Schemas-and-Artifacts).  
...
* Magic-10 IDs & order: PF12-Canon-HDE-Schemas-and-Artifacts §2.6, PF01-Canon-HDE-Math-Spec §5.1.  
    
* Public Reader contract: PF05-Canon-HDE-CLI-API-Vendor-Ref.  
    
* Canonical JSON & pack: PF12-Canon-HDE-Schemas-and-Artifacts §4.  
    
* Governance tokens: HDE-Governance — §2.0 Acceptance Tokens.
...
* Score function. Deterministic fixed-point combination across the ten categories (integer path), consistent with PF01-Canon-HDE-Math-Spec rounding/banding (titles-only).  
...
* Evidence Index: append artifacts (sampler snapshots, seed replay logs) in the same PR; PF12-Canon-HDE-Schemas-and-Artifacts machine mirror lines present with path-proofs.  
...
* Scoring/banding/rounding rules: PF01-Canon-HDE-Math-Spec.  
    
* Public contract & transport: PF05-Canon-HDE-CLI-API-Vendor-Ref, HDE-Governance (A7).  
    
* Canonical JSON rules: PF12-Canon-HDE-Schemas-and-Artifacts §4.
...
* sample\_and\_rank — the main helper used by dev/admin harnesses to build the pool and produce a ranked list of candidates.
...
Purpose. Provide a central formatter for typed, numeric-free errors. Public error transport (writers/errors no-store, headers) lives in HDE-Governance and PF05-Canon-HDE-CLI-API-Vendor-Ref (titles-only). Identity/Meta content has been moved to §13 (Identity & Provenance Module) and §14 (Internal Meta Surface) to avoid duplication.

## **12.1 Error envelope (normative)**

The governed HTTP error envelope, canonical bytes, and deterministic mapping are defined in PF05-Canon-HDE-CLI-API-Vendor-Ref §5.2.

* Numeric-free & secret-free: never include stack traces, payload excerpts, header values, or secrets; keys-only logs (redacted).

## **12.2 Token set (titles-only)**

The canonical HTTP error token vocabulary and mapping for PF05-owned surfaces are defined in PF05-Canon-HDE-CLI-API-Vendor-Ref §5.2.

## **12.3 Transport (titles-only)**

HTTP error transport and the distinction between HTTP error bodies and CLI failure tokens are defined in PF05-Canon-HDE-CLI-API-Vendor-Ref §§3.3 and 5.2.

## **12.4 Acceptance (binary; tokens & evidence are titles-only)**

Acceptance for PF05-owned error surfaces is defined in PF05-Canon-HDE-CLI-API-Vendor-Ref §5.2; acceptance-token semantics remain in HDE-Governance.

## **12.5 Evidence (records-only; path-agnostic; indexed via PF12-Canon-HDE-Schemas-and-Artifacts machine mirror)**
...
* artifact\_key: parity/errors\_reader\_cli — token-level parity for the same error across HTTP and CLI.

Machine Mirror record fields, semantics, and order are defined in PF12-Canon-HDE-Schemas-and-Artifacts §8.3. Update the Human Evidence Index and Machine Mirror in the same change; CI enforces 1:1 parity and path-proofs.

Routing (titles-only):

* Error shapes & code map ownership: PF05-Canon-HDE-CLI-API-Vendor-Ref.  
    
* Writers/errors transport posture: HDE-Governance.  
    
* Canonical JSON & mirror rules: PF12-Canon-HDE-Schemas-and-Artifacts §4/§8.  
...
Source of truth (titles-only): release\_id derives only from the PF12-Canon-HDE-Schemas-and-Artifacts pack manifest; Invocation tag and digest come from the governed Invocation record, with the digest recomputed from the UTF-8 tag bytes; engine\_tag, build\_commit, and emitter\_sha256 are taken from the build snapshot at cut. No request-time hashing.

## **13.2 Accessors**

* identity\_meta() → {"engine\_tag","invocation\_tag","release\_id"} — inserted into the public envelope before idempotence hashing (PF01-Canon-HDE-Math-Spec §3.2).  
...
* Preimage coupling. identity\_meta() enters the public preimage before idempotence\_hash is computed (PF01-Canon-HDE-Math-Spec §3.2).  
...
Scope (normative). Identity artifacts are records-only and MUST be listed by title/path in the Human Evidence Index (docs/evidence/INDEX.json) and mirrored 1:1 in artifacts/evidence\_index.jsonl. Machine Mirror record fields, semantics, and order are defined in PF12-Canon-HDE-Schemas-and-Artifacts §8.3. Update the Human Evidence Index and Machine Mirror in the same commit/PR; CI fails on mismatch or missing path-proofs. All captures run with LC\_ALL=C, LANG=C, TZ=UTC.
...
* Pack/manifest & release\_id: PF12-Canon-HDE-Schemas-and-Artifacts §6.  
...
* perspective (exactly one of: `shared`, `a_to_b`, `b_to_a`)  
    
* viewer\_top  
    
* flags

Output: { personal\_key, shared\_key } — both drawn from the Narrative Key Registry. The output-field names are not perspective values: `shared` selects `shared_key`; `a_to_b` and `b_to_a` select `personal_key`, with direction applied at render time.
...
* Authoring DB plane (intake, lints, preview) and runtime file-backed pack (sealed; no DB in hot path) live in PF17-Canon-HDE-Narratives-Guide / PF12-Canon-HDE-Schemas-and-Artifacts.  
...
* Versioned registry guarantees exactly one `personal_key` and one `shared_key` per `(category, band)` row.  
...
* Authoritative contract: the CLI must conform end-to-end to PF05-Canon-HDE-CLI-API-Vendor-Ref for commands/flags, payload shapes, error model, streams/exits, and help/version formatting (titles-only; PF05-Canon-HDE-CLI-API-Vendor-Ref governs).  
    
* Admin preview posture. The narrative preview surface is enabled by default for admins across dev/stage/prod and uses the same emitter as Aux; bytes parity and LF discipline apply. Bytes and route live in PF05-Canon-HDE-CLI-API-Vendor-Ref by title.

## **16.1 Command catalog (titles-only; PF05-Canon-HDE-CLI-API-Vendor-Ref governs)**

Single home for commands. The complete CLI command/flag catalog and their status live in PF05-Canon-HDE-CLI-API-Vendor-Ref (for example, “Commands (by status)”, “CLI Overview & Conventions”). This guide does not enumerate all commands.

Conformance expectation. CLI help/usage, flags, and behavior must match PF05-Canon-HDE-CLI-API-Vendor-Ref. Any divergence is a defect until corrected.
...
* hdctl sample \<ARGS\> — prints the same deterministic selection/ordering as the corresponding Reader surface documented in PF05-Canon-HDE-CLI-API-Vendor-Ref.  
    
* Additional commands (for example, read singlebg, list people, disabled fetch \*) are defined and governed in PF05-Canon-HDE-CLI-API-Vendor-Ref. This document only illustrates conformance expectations.
...
* Exit code. MUST be non-zero on failure and MUST be consistent with the governing command contract in PF05-Canon-HDE-CLI-API-Vendor-Ref §3.3.  
...
* Canonical JSON. UTF-8 (no BOM), ASCII-sorted keys, compact separators, one LF; arrays-as-sets deduped and ASCII-sorted (see §4).  
...
* Allow-list. CLI reads only documented env and fails-closed on unknowns (see §32.2).
...
List by title/path in the Human Evidence Index (docs/evidence/INDEX.json) and mirror 1:1 in artifacts/evidence\_index.jsonl. Machine Mirror record fields, semantics, and order are defined in PF12-Canon-HDE-Schemas-and-Artifacts §8.3. Update the Human Evidence Index and Machine Mirror in the same PR.
...
* CI rails posture. CI runs CLOSED by default. Any job that opens rails must pin retry/timeout/backoff policy and must index all governed evidence in the same PR (titles-only routing to PF04-Canon-HDE-Governance and PF12-Canon-HDE-Schemas-and-Artifacts).
...
* /internal/version is ops-only and not A7-eligible (see §9.4; PF04-Canon-HDE-Governance §10.5).
...
(Bytes/policy routed by title to PF05-Canon-HDE-CLI-API-Vendor-Ref and PF04-Canon-HDE-Governance)

## **25.3 Orchestration & ordering**

The harness can be invoked standalone or as part of the sanity pipeline (see §27.5).
...
## **25.4 Acceptance (titles-only; token names live in PF04-Canon-HDE-Governance §2.0)**
...
* Aux/Narrative (EPIC-010). Acceptance is governed by PF04-Canon-HDE-Governance and PF19-Canon-Glow-QA-Guide (titles-only). PF14 does not list token names. Mechanics requires that Aux evidence capture and suppression posture are provable via the limited headers-only snapshots and any associated parity checks defined in the governing docs.

## **25.5 Evidence & indexing (records-only; machine mirror; same-PR rule)**

Single home for titles/paths. §37 Documentation Artifacts and Registry (“Reader success catalog & A7 proofs”). This guide does not pin file paths here.

Indexing:

* List artifact titles/paths in the Human Evidence Index (docs/evidence/INDEX.json).  
...
Mirror field order note. Mirror field order and CI tokens are pinned once in §1.3; this section routes by exact title to PF12-Canon-HDE-Schemas-and-Artifacts §8.3 for the owned field order and gates.

## **25.6 Routing (titles-only)**

* Transport matrices & A7 policy: PF05-Canon-HDE-CLI-API-Vendor-Ref; PF04-Canon-HDE-Governance.  
    
* Domains & catalogs / canonical JSON rules: PF12-Canon-HDE-Schemas-and-Artifacts.  
    
* Math semantics (preimage, bands, comparators): PF01-Canon-HDE-Math-Spec.  
    
* Ops endpoint posture: PF04-Canon-HDE-Governance §10.5 (see §9.4 for PF14 ops block).
...
Routing: SLO targets and failure posture live in PF04-Canon-HDE-Governance (titles only).

# 27\) Release and Provenance Packaging

Purpose (normative). Use one canonical manifest as the tracked release-identity input, derive release\_id from those bytes, and build current release-bound derivatives only through an isolated external attestation. Manifest shape and canonical rules live in PF12-Canon-HDE-Schemas-and-Artifacts (titles only).
...
Acceptance is governed by PF04-Canon-HDE-Governance and pack/manifest rules in PF12-Canon-HDE-Schemas-and-Artifacts (titles-only). This section creates no token, QA PASS, OPS completion, PF09 status movement, deployment, migration, or closeout claim.

Routing (titles-only). Manifest shape, canonical JSON rules, historical evidence schemas, and external-attestation schemas: PF12-Canon-HDE-Schemas-and-Artifacts. Public transport remains in PF05-Canon-HDE-CLI-API-Vendor-Ref / PF04-Canon-HDE-Governance.
...
Routing. Transport matrices live in PF05-Canon-HDE-CLI-API-Vendor-Ref; /internal/version policy in PF04-Canon-HDE-Governance §10.5; DB mechanics in §20.
...
List by title/path in the Human Evidence Index (docs/evidence/INDEX.json) and mirror it 1:1 in artifacts/evidence\_index.jsonl (canonical JSONL; one LF). Machine Mirror record fields, semantics, and order are defined in PF12-Canon-HDE-Schemas-and-Artifacts §8.3.
...
## **28.7 Acceptance (titles-only; tokens live in PF04-Canon-HDE-Governance §2.0)**

Success route (A7). Governed by the A7 token family in PF04-Canon-HDE-Governance (200 ETag, HEAD parity, 304 omission, success cache/Vary).
...
This guide asserts capability-level conformance and routes all token names to PF04-Canon-HDE-Governance §2.0. Artifacts are captured and indexed per §1.3 and §37.
...
Single binary & emitter. Wire the allow-listed presenter/emitter (see §4 and §8.2); forbid ad hoc serialization in entrypoints.
...
Reader A7 (public). See PF05-Canon-HDE-CLI-API-Vendor-Ref and PF04-Canon-HDE-Governance (ETag over LF-terminated body; Vary policy; 304 omits Content-Type and Content-Length; HEAD parity; POST non-conditional).
...
Identity. release\_id from canonical manifest (PF12-Canon-HDE-Schemas-and-Artifacts §6); invocation\_tag participates in preimage (PF01-Canon-HDE-Math-Spec §3); presenter uses canonical serializer (§4).
...
Acceptance token naming and semantics are owned by PF04-Canon-HDE-Governance (titles-only). This guide does not list token names. The acceptance obligations in §32 include start-command capture, app factory correctness, binding to $PORT, environment allow-list discipline, and ops posture for /internal/version, all proven via governed evidence.

## **32.7 Evidence (titles/paths only)**

Single home for artifact titles/paths. Do not pin file paths here. The authoritative registry of artifacts and their titles/paths lives in §37 Documentation Artifacts and Registry.

Mechanics MUST:

* List artifacts by title/path in the Human Evidence Index (docs/evidence/INDEX.json).  
    
* Write records-only mirror entries to artifacts/evidence\_index.jsonl in the same commit/PR (machine).  
    
* Ensure each mirror record is canonical JSONL (UTF-8, no BOM; sorted keys; compact; exactly one LF). Machine Mirror record fields, semantics, and order are defined in PF12-Canon-HDE-Schemas-and-Artifacts §8.3.  
    
* Maintain strict 1:1 parity between human Index and machine mirror; CI fails on mismatch or missing path-proofs.

Routing (titles-only). A7 and ops policies: PF04-Canon-HDE-Governance. Public bytes & preimage: PF05-Canon-HDE-CLI-API-Vendor-Ref / PF01-Canon-HDE-Math-Spec. Pack/manifest: PF12-Canon-HDE-Schemas-and-Artifacts. Evidence registry/mirror discipline: §1.3 and §37.
...
**Runner and scope.** The canonical dev HTTP capture runner is `scripts/dev_start_reader.sh`, which invokes `python -m adapter.http_reader`. It is for deterministic dev evidence capture, not public infrastructure. This section inventories a bounded capture context; it does not replace the authoritative Endpoint Catalog owned by PF12-Canon-HDE-Schemas-and-Artifacts, declare omitted adapter routes retired, or prove reachability, Catalog membership, A7 eligibility, implementation completion, or public authorization. Dev-harness evidence remains supplemental. `/internal/version` remains ops-only.

**Required local capture profile.**

* Bind ordinary local capture to `127.0.0.1`. Do not expose this profile publicly.  
    
* Use port `8000` by default. An explicitly supplied `PORT` may override it; captures and examples MUST use the effective port.  
    
* Require the caller to supply `APP_ENV=dev`. Do not document a missing value as equivalent to dev.  
    
* Keep rails closed with `SAFE_MODE=1` and `ALLOW_NETWORK=0`; captures perform no vendor or unrelated network I/O.  
    
* Pin `LC_ALL=C`, `LANG=C`, and `TZ=UTC`.  
    
* Disable CORS. No cross-origin allow-list, wildcard, credentials, or CORS response headers are authorized for this harness.  
    
* Set Flask `debug=False`, `use_debugger=False`, and `use_reloader=False` explicitly.

**Routes in the bounded capture context.**

| Route | Method posture | Classification and constraint |
| :---- | :---- | :---- |
| `/reader?v=1` | `GET`, `HEAD` | Canonical Reader v1 dev/proof surface. It requires explicit `APP_ENV=dev`; the Endpoint Catalog owned by PF12-Canon-HDE-Schemas-and-Artifacts retains the Reader success-proof designation. |
| `/api/reader?v=1` | `GET`, `HEAD` only when the Reader blueprint is mounted under `/api` | Conditional alias of `/reader`, never a second contract or proof surface. |
| `/api/compat/v1` | `GET`, `POST`, `HEAD`, `OPTIONS` | Internal/admin compatibility surface. `GET` is the fixed bodyless probe; `POST` performs evaluation. The other methods follow the exact PF05-Canon-HDE-CLI-API-Vendor-Ref posture. |
| `/aux/narrative?v=1` | `GET` | Canonical public Aux narrative surface. Co-location does not make it dev-only or A7-eligible. |
| `/api/aux/narrative?v=1` | `GET` only when the configured alias is exposed | Conditional alias of the canonical Aux contract, not an independent public surface. |
| `/internal/dev/sampler` | `POST` only | Dev/admin-only internal sampler. It requires `APP_ENV` in `{dev,test,local}`, runs with closed rails, returns writer-style `no-store` responses, and is not A7-eligible. |

PF05-Canon-HDE-CLI-API-Vendor-Ref owns each route's exact request, response, status, header, conditional, and error contract. PF12-Canon-HDE-Schemas-and-Artifacts owns the authoritative Endpoint Catalog.

**Error, serializer, and logging posture.**

* Failures use the route-specific non-2xx status and typed error contract owned by PF05-Canon-HDE-CLI-API-Vendor-Ref; this section defines no competing dev error envelope.  
    
* Where PF05-Canon-HDE-CLI-API-Vendor-Ref requires JSON, emit canonical UTF-8 JSON through the allow-listed shared presenter/emitter and terminate it with exactly one LF when the owning contract requires it.  
    
* Keep diagnostics and logs separate from response bytes. Capture logs contain no secrets, credentials, birth data, narrative prose, or payload bodies.  
    
* Preserve every route-specific `Cache-Control`, `ETag`, method, conditional, numeric-free, public/private, and A7/non-A7 boundary.

**Required start and local examples.** After the runner is hardened to the required profile, start the ordinary local capture with:

```shell
LC_ALL=C LANG=C TZ=UTC SAFE_MODE=1 ALLOW_NETWORK=0 APP_ENV=dev PORT=8000 scripts/dev_start_reader.sh
```

This command is a documentation contract, not a claim that the current server bind satisfies the required loopback posture. For an explicit `PORT` override, replace `8000` in each local base URL with the effective port.

* Bodyless compatibility probe: `curl -s http://127.0.0.1:8000/api/compat/v1 | jq .`  
    
* Reader v1: `curl -s 'http://127.0.0.1:8000/reader?v=1&a=<A_CHART_PATH>&b=<B_CHART_PATH>&a_tz=<A_TZ>&b_tz=<B_TZ>' | jq .`  
    
* Internal sampler: `curl -s http://127.0.0.1:8000/internal/dev/sampler -H 'Content-Type: application/json' -X POST -d '{"viewer_id":"<VIEWER_ID>","candidate_ids":["<CANDIDATE_ID>"],"seed":"<SEED>"}' | jq .`

**Shared environments.** Use only the infrastructure-provided private or authenticated start command, bind, forwarded base URL or `DEV_SAMPLER_URL`, and effective port. Do not compute, guess, or hard-code a shared-environment host or port. A shared-environment `0.0.0.0` bind is permissible only behind that environment's access-controlled forwarding boundary; it does not authorize public exposure or change any route classification.

**Current repository posture at `d7e8b4e7f07a844fd976dcd68a8cabe983891e75` (static inspection only).**

* `scripts/dev_start_reader.sh` invokes `python -m adapter.http_reader`, defaults `PORT=8000`, `SAFE_MODE=1`, `ALLOW_NETWORK=0`, `LC_ALL=C`, `LANG=C`, and `TZ=UTC`, and propagates `APP_ENV` only when the caller supplies it.  
    
* `adapter/http_reader.py` binds `0.0.0.0`, not the required ordinary-local `127.0.0.1`; the loopback requirement is not currently satisfied.  
    
* The module invocation does not explicitly bind `debug=False`, `use_debugger=False`, or `use_reloader=False`; explicit capture controls remain required.  
    
* The selected factory contains no explicit CORS binding or test-backed disabled-CORS declaration; the disabled posture remains a requirement.  
    
* The Reader handler treats missing `APP_ENV` as dev; that behavior does not satisfy the explicit caller-supplied `APP_ENV=dev` requirement.  
    
* `docs/ENDPOINTS_CATALOG.json` includes the Reader `GET`/`HEAD` rows and `POST /internal/dev/sampler`, but it does not express the complete current PF05-Canon-HDE-CLI-API-Vendor-Ref and PF12-Canon-HDE-Schemas-and-Artifacts route-and-alias projection. Catalog conformance is not claimed.  
    
* `.devcontainer/devcontainer.json` supplies `DEV_SAMPLER_URL=http://127.0.0.1:8000/internal/dev/sampler` for the Codespaces container. This statically confirms that one Codespaces binding only; other shared environments must use their own infrastructure-provided value.
...
Every primary governed artifact listed in this registry MUST have exactly one record in `artifacts/evidence_index.jsonl`. A required sibling `*.path_proof.txt` transcript is linked from the primary record through `proof_anchor` and MUST NOT receive a separate Machine Mirror row unless HDE-Schemas & Artifacts explicitly catalogs that transcript as an independent primary governed artifact.
...
Historical EPIC022 release evidence (frozen capture-time records):

* `artifacts/math/freeze_pack_manifest.json`  
* `artifacts/math/release_id.txt`  
* `artifacts/math/release_id_recompute.log`

Release-ID evidence path canonicalization for this retained historical family (normative). The canonical retained paths are:

* `artifacts/math/release_id.txt`  
* `artifacts/math/release_id_recompute.log`

References to `audit/gates/release/release_id.txt` or similar `audit/gates/release/**` paths are deprecated and MUST NOT be used for evidence indexing or close-pack checks. If a transitional copy is required, it MUST be mechanically generated from the canonical `artifacts/math/**` source without manual editing, and indexing MUST reference the canonical `artifacts/math/**` paths.

Checksums audit:

* `artifacts/math/checksums_audit.log`

These paths preserve the manifest bytes, release ID, recompute trace, and checksum audit from their original capture. They are not current runtime identity inputs and MUST NOT be interpreted as mirrors of later `catalog/manifest.json` bytes. Current release identity derives from `catalog/manifest.json`; current release-bound derivatives are produced only through the external attestation mechanics in §27.3.
...
Historical DB bridge and provider-parity evidence (retained records only):

The artifacts in this subsection are frozen bridge-era records governed by §20.3.2. They do not prove current bridge availability, runtime support, fallback, direct-versus-bridge parity, bridge consistency, acceptance-token satisfaction, or a current OPS result.
