...
**Current controlling posture:** Prove `DATABASE_URL` and `psycopg` only; every retired transport key refuses before provider construction or I/O; missing or unavailable direct access fails closed with typed posture; alternate-provider attempts are zero; live direct posture is read-only; DDL truth is projection-only through `hde.ddl_identity_projection.v1`; and current local plus OPS-03 evidence is bound in the Human Evidence Index and Machine Evidence Mirror. The bridge-specific description and artifact list that follow are retained as historical OPS-01 context only and are not active completion obligations.
...
**Projection dependency.** The mapped-cache owner must consume `project_bodygraph()` from `engine/bodygraph/projection.py` before persistence and again for canonical read-back comparison. This dependency does not move persistence into PR-04, change the recorded Optional status, or authorize production writes.

**Subtask status:** Optional

**Epic or card:** Future epic, pending PO / Lead Dev authorization.
...
This is a Distillation candidate because it hardens durability, repeatability, and DB/cache read-back practice after the HDE-EPIC037 dry-run evidence chain. It is not required unless PO / Lead Dev decides durable configured-v2 BodyGraph cache writes are now in scope.

Current configured-v2 `bg:resolve --source vendor --dry-run` may use the version-neutral `charts` route and deterministic v2 ChartResult adapter, but configured-v2 non-dry-run mapped-cache writes remain fail-closed until this or an equivalent mapped-cache persistence slice proves safe persistence.

This subtask must not simply turn on writes. It must prove a safe mapped-cache persistence path that writes adapter-mapped HDE data, not raw v2 vendor envelopes.

This subtask does not claim mapped-cache persistence exists now, configured-v2 non-dry-run writes are supported now, `bg:resolve --source vendor --upsert` is production-authorized, public Reader behavior changed, any new public route, flag, payload, transport, or HTTP home exists, app-side HumanDesignAPI ownership exists, QA PASS, PF09 status movement, OPS completion, PO closeout, board update, production deployment, epic closeout, or broad HumanDesignAPI v2 platform conformance.
