...
* External `hde.release_attestation.v1` bundle — current release provenance produced outside tracked source and bound to the exact source commit, deterministic tracked-tree digest, canonical manifest and `release_id`, sorted file inventory with hashes and sizes, and canonical checksums.

The external bundle is not inserted into tracked source or used to regenerate tracked release derivatives, the Human Evidence Index, or the Machine Mirror.

**Historical checked-in evidence (frozen capture-time records):**

* `artifacts/math/freeze_pack_manifest.json` (+ `.sha256`) — historical evidence copy of `catalog/manifest.json`  
* `artifacts/math/release_id.txt` — historical `sha256(canonical_manifest_bytes)` capture  
* `artifacts/math/release_id_recompute.log` — historical recomputation trace  
* `artifacts/math/checksums_audit.log` — historical per-entry path/sha256/size verification

These checked-in records are not current release-identity inputs or current release attestations. Do not regenerate, relabel, or refresh them merely because the manifest or `release_id` changes.

**Must prove:** the canonical manifest has the exact required structure and bytes; `files[]` is ASCII-sorted and unique; recorded digests and sizes match; recomputation yields the manifest-derived `release_id`; and current release provenance preserves the acyclic dependency from tracked source through the manifest and `release_id` to external attestation.
