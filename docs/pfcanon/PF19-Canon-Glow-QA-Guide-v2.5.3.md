...
* Copy or paste artifacts, presentation escapes, AI-rendered escapes, markdown-rendered escapes, transcript-formatting escapes, quote-formatting escapes, and assistant-output escapes are non-blocking review artifacts by default, including when they appear in machine-sensitive strings such as paths, commands, tokens, environment variables, endpoint strings, JSON keys, evidence filenames, manifest filenames, hash filenames, path-proof filenames, shell snippets, Python expressions, heredoc markers, or QA-created script bodies.  
* Reviewers MUST ignore the apparent escape layer and evaluate the underlying command identity, artifact identity, path identity, token identity, evidence identity, proof target, and PASS, FAIL, or TOOLING posture.  
* A reviewer MUST NOT raise a Blocker whose primary issue is rendered escape characters, rendered backslashes, markdown-escaped underscores, escaped shell redirection, escaped heredoc syntax, escaped command punctuation, escaped Python syntax, or a claim that a command is not runnable because the reviewer’s rendered quote shows escapes.  
* A Blocker is allowed only for a substantive non-rendering defect that remains material after the apparent escape characters are removed, such as missing or ambiguous command identity, unproven repo locus, missing required deliverable, missing PASS or FAIL criterion, unsafe public-surface or provider-rails posture, canon conflict, or a proof target that remains unclear after ignoring rendering escapes.  
* The burden is on the reviewer to prove the substantive non-rendering defect. The plan author does not need to prove that AI-added escape characters are fake.  
* If the intended proof target remains clear after ignoring rendered escapes, the correct disposition is no issue, Suggestion, or Caveat according to execution risk. It is not a Blocker.  
...
* Coverage vs QA Plan accounting MUST separately identify any accepted plan-execution deviation that materially changed how a COVERED step was actually run, including bounded Moon Loop reruns, rails changes, and step-local dependency-preflight corrections. For each such deviation, the closeout record MUST name the affected step, state the accepted deviation, and point to the governed evidence that preserves the deviation and the final accepted basis.
