...
* **Source-neutral BodyGraph convergence.** Source-invariance evidence compares independently mapped configured-v2 vendor and mapped-cache inputs only after both pass through `project_bodygraph()`; the shared Presenter remains the sole public-byte emitter. Projection itself creates neither persistence nor public bytes, and concrete fixtures, schemas, proof predicates, and artifact identities remain in their owning documents.  
* **Current direct-only DB evidence vs historical bridge evidence.** Current database-selection evidence proves only direct psycopg selection through `DATABASE_URL`, fail-closed missing or unavailable direct access, retired-key refusal before provider attempts, and zero alternate transports. Retained bridge-era evidence is historical provenance only and cannot prove current service availability, runtime support, fallback, direct-versus-bridge parity, a current direct-only release gate, or current OPS PASS.  
