...
**Historical-only bridge evidence.** The bridge-era files and proof chains listed in D.11aa through D.11ac are historical records of the contract that governed their capture. They MUST NOT be used to claim current `pg-bridge` availability or support, direct-versus-bridge parity, bridge fallback or consistency, successful bridge selection, current token satisfaction, or a current OPS PASS. Accurate historical occurrences remain historical only; their bytes, hashes, path proofs, Human Evidence Index bindings, and Machine Mirror bindings remain unchanged unless a separately authorized historical-evidence migration changes them.
