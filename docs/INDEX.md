# HD Engine Repo Docs — Index (HDE-EPIC026 aligned)
...
- EPIC026 QA ledger: `audit/qa/hde-epic026/qa_step_logs_manifest.json` plus per-check logs under `audit/qa/hde-epic026/checks/<check_id>/primary.log` (preflight + gate checks). Close-pack: `audit/EPIC-026_MANIFEST.json`, `audit/EPIC-026_close_report.md`, doc deltas in `audit/docdeltas/hde-epic026_doc_deltas.md`.
...
- Conjunction CLI governance: conjunction captures under `artifacts/audit/cli/` and `artifacts/cli/abba_sidecar.json` are indexed in the Evidence Index/Mirror and included in canonical JSON gate target expansion.
...
- Evidence-path and LF-ending gates: `python tools/evidence/validate_evidence_paths.py` and `python tools/evidence/check_lf_endings.py` (logs captured under `audit/qa/hde-epic026/checks/`).
