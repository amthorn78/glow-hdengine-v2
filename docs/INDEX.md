...
## Acceptance & evidence
- EPIC022 evidence: showcompat D2 captures (`artifacts/cli/showcompat/stdout.json`, `.sha256`, `args.json` generated by `tools/cli/generate_showcompat_artifacts.py`), env-pin and refusal parity evidence (`audit/gates/determinism/env_pins.log`, `parity/errors_reader_cli.vendor_attempt_closed_rails.*`), `/internal/version` D3 bundle (`artifacts/ops/internal_version/{body_get.json,body_get.sha256,headers_get.txt,headers_head.txt,cond_if_none_match_headers.txt,cond_if_modified_since_headers.txt,two_run_identity.log}`), acceptance map (`docs/acceptance_map_epic022.json`), token evidence matrix (`audit/qa/hde-epic022/token_evidence_matrix.md`), and finalized close-pack artifacts (`audit/EPIC-022_MANIFEST.json`, `audit/EPIC-022_close_report.md`, `artifacts/sanity/sanity.log` + `.path_proof.txt`).
