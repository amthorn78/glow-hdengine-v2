...
- Generic BodyGraph ingest and non-dry-run mapped-cache writes remain guarded from raw v2 ChartResult persistence until a safe mapped-cache persistence path exists. OPS-01 was a bounded PO-only open-rails dry-run smoke; docs or PR work must not rerun OPS or treat it as OPS completion.
