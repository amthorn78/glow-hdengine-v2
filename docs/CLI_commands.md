...
- Guard outputs, QA dumps, and other governed artifacts must have `.path_proof.txt` siblings plus entries in `docs/evidence/INDEX.json` and `artifacts/evidence_index.jsonl`. `python tools/evidence/update_evidence_index.py` is the sole active writer for the complete evidence skeleton, including orientation and proof companions.
...
- Orientation and sanity checks: use `python tools/evidence/update_evidence_index.py --check` and `python tools/evidence/orientation_demo.py --check` for read-only skeleton validation. `python tools/evidence/run_sanity_pipeline.py` writes the canonical `audit/gates/sanity_pipeline/sanity_pipeline.log` under closed rails and is run only when release-sanity work is explicitly scoped. `artifacts/sanity/sanity.log` is retained only as historical, unbound material and is never refreshed; do not use it for a current result. Run `ci/checks/check_mirror_schema.sh` to validate mirror schema/self-record/path-proof discipline.
- Canonical JSON gate (closed rails): `python tools/evidence/run_canonical_json_gate.py` writes `audit/gates/json_gate/canonical/json_gate_check_log.ndjson`, `audit/gates/json_gate/canonical/json_gate_compare_log.ndjson`, and `audit/gates/json_gate/canonical/json_gate_structured_record.json` with `.path_proof.txt` siblings; `python tools/evidence/run_canonical_json_gate.py --check-only` is the read-only form used by generic release-sanity stage 03 when the change-aware release lane applies. `.github/workflows/ci.yml` has no standalone step named “Run canonical JSON gate (closed rails).” The legacy catalog check report remains at `audit/gates/canonical_json/json_canonical_check.log`.
...
- Evidence index updater: `python tools/evidence/update_evidence_index.py` publishes registry_report, sanity, showcompat, and other governed bindings into `docs/evidence/INDEX.json`, `docs/evidence/INDEX.sha256`, `artifacts/evidence_index.jsonl`, `artifacts/evidence_index.jsonl.sha256`, orientation, and proof companions. After that single write transaction, run `python tools/evidence/update_evidence_index.py --check`, `python tools/evidence/orientation_demo.py --check`, and `ci/checks/check_mirror_schema.sh` read-only.
- Release identity (closed rails): `catalog/manifest.json` is the sole Git input and `release_id = sha256(canonical_bytes(manifest))`. Validate it read-only with `python scripts/release_id_recompute.py --check-manifest-only`; cut a release explicitly with `python scripts/cut_release_manifest.py --version <semver> --built-at-utc <UTC>`; build current evidence with `python tools/evidence/build_release_attestation.py --output <external-empty-directory> --require-clean`. CI publishes the strict bundle outside Git. Existing checked-in EPIC022 release evidence remains frozen historical material and is not regenerated per release.
