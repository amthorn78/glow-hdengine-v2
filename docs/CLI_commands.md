...
- Orientation and sanity checks: `python tools/evidence/orientation_demo.py` and `python tools/evidence/run_sanity_pipeline.py` (pipeline emits `artifacts/sanity/sanity.log` under closed rails). Run `python ci/checks/check_mirror_schema.sh` to validate mirror schema/self-record/path-proof discipline (Python entrypoint despite `.sh` suffix).
- Canonical JSON gate (closed rails): `python tools/evidence/run_canonical_json_gate.py` (CI step “Run canonical JSON gate (closed rails)”) writes `audit/gates/canonical_json/json_canonical_check.log`, `audit/gates/canonical_json/json_canon_compare.log`, and `audit/gates/canonical_json/canonical_json.gate.json` with `.path_proof.txt` siblings (`--check-only` available for read-only validation).
