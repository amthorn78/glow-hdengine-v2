...
- `hdctl bg:resolve --user <user> [--source auto|db|vendor] [--birthdate YYYY-MM-DD --birthtime HH:MM --location <place>]`
  - Closed rails (`SAFE_MODE=1`, `ALLOW_NETWORK=0`) refuse vendor resolution before route-policy classification, client construction, request construction, fetch, ingest, DB, DNS, socket, or HTTP. Under explicitly open rails, configured v2 bases now use the governed, version-neutral `charts` route plus the deterministic v2 ChartResult adapter for `--dry-run` resolver output; generic ingest and non-dry-run writes remain fail-closed until mapped-cache persistence is implemented. Non-v2 configured bases preserve explicit legacy BodyGraph fallback.
...
- EPIC036 records historical pre-adapter route-policy evidence: configured v2 bases were classified as `unsupported_runtime_nonclaim` before constructing a legacy `bodygraphs` request, and explicit legacy fallback was preserved only for non-v2 configured bases. Current EPIC037 PR-03 resolver wiring supersedes that configured-v2 runtime posture for dry-run use by selecting `charts` with `Authorization: Bearer <redacted>` plus `HD-Geocode-Key: <redacted>` and mapping through the deterministic v2 ChartResult adapter. Legacy BodyGraph route metadata retains `HD-Api-Key: <redacted>` posture for non-v2 fallback.
