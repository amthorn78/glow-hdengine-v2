Got it. Using your v1.2 as the baseline (I read the attached file) , here’s a clean v1.3 you can paste in. It keeps all A3–A5 behavior the same, adds the explicit A5 transport guard, and appends a small A6 acceptance consolidation marker list (names only).
...
idempotence_hash = sha256( sercanon(preimage_without_idempotence_hash) ).hexdigest()  # lowercase 64-hex
...
> A5 transport guard: Reader v1 (A5) does not emit ETag or Cache-Control; conditional GET and transport caching arrive in A7.
...
No ETag or Cache-Control; no conditional 304. These are introduced in A7.
...
6. A6 — acceptance consolidation checklist (markers only)

Add these marker names to your evidence when closing A6 tasks. They consolidate checks without changing A3–A5 behavior.

Determinism & parity

CLI_AB_BA_IDENTITY=OK

READER_AB_BA_IDENTITY=OK
...
SERIALIZER_UNIQUE=OK

EMITTER_SHA256_OK


Preimage & release identity

PREIMAGE_HASH_OK

RELEASE_ID_BODY_MATCH=OK


Harness gating & correctness
...
A5 success guard (no transport in A5)
...
7. Signoff template (paste in PO closeout)
