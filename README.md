...
- Machine mirror: `artifacts/evidence_index.jsonl`; governed artifacts include `.path_proof.txt` siblings (mirror path proof includes both file sha256 and `mirror_body_sha256`).
...
- `/internal/version` D3 artifacts: `artifacts/ops/internal_version/{body_get.json,body_get.sha256,headers_get.txt,headers_head.txt,cond_if_none_match_headers.txt,cond_if_modified_since_headers.txt,two_run_identity.log}` (all with `.path_proof.txt`).
- Orientation and sanity checks: `python tools/evidence/update_evidence_index.py` (write) then `python tools/evidence/orientation_demo.py` (write) followed by their `--check` variants; `python tools/evidence/run_sanity_pipeline.py` (closed rails).
