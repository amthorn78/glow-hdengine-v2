# Glow HD Engine — EPIC023 documentation sweep
...
- What EPIC023 adds:
  - Canonical JSON gate artifacts live under `audit/gates/canonical_json/` (`json_canonical_check.log`, `json_canon_compare.log`, `canonical_json.gate.json` plus `.path_proof.txt` siblings) and are produced by `python tools/evidence/run_canonical_json_gate.py` (CI step “Run canonical JSON gate (closed rails)”).
  - Acceptance scaffolds and QA ledger: `docs/acceptance_map_epic023.json`, `audit/qa/hde-epic023/token_evidence_matrix.md`, `audit/qa/hde-epic023/acceptance_map_viability.log`, `audit/qa/hde-epic023/qa_step_logs_manifest.json`, and the doc-delta + PF23 consult pair (`audit/docdeltas/hde-epic023_doc_deltas.md`, `audit/qa/hde-epic023/00_meta/pf23_consult.md`).
  - Close-pack artifacts exist and are governed: `audit/EPIC-023_MANIFEST.json`, `audit/EPIC-023_close_report.md`. The acceptance set is the canonical 8-token roster (`QA_ACCEPTANCE_MAP_VIABILITY_OK`, `EVIDENCE_INDEX_MIRROR_OK`, `EVIDENCE_PATHS_VALIDATED_OK`, `SANITY_PIPELINE_OK`, `DETERMINISM_ENV_PINS_OK`, `JSON_CANONICAL_CHECK_OK`, `DOC_DELTA_PRESENT_OK`, `TWO_RUN_IDENTITY_OK`).
...
- Evidence skeleton (update in the same PR whenever governed bytes change): `docs/evidence/INDEX.json`, `docs/evidence/INDEX.sha256`, `artifacts/evidence_index.jsonl` (+ `.path_proof.txt` siblings). Refresh with `python tools/evidence/update_evidence_index.py` → `python tools/evidence/orientation_demo.py` → their `--check` variants → `ci/checks/check_mirror_schema.sh` (Python entrypoint).
- Canonical JSON gate (closed rails, enforced in CI): `python tools/evidence/run_canonical_json_gate.py` emits `audit/gates/canonical_json/json_canonical_check.log`, `audit/gates/canonical_json/json_canon_compare.log`, and `audit/gates/canonical_json/canonical_json.gate.json` with co-located path proofs; CI runs this as “Run canonical JSON gate (closed rails)” in `.github/workflows/ci.yml`.
- Acceptance + QA ledger (EPIC023): `docs/acceptance_map_epic023.json`, `audit/qa/hde-epic023/token_evidence_matrix.md`, `audit/qa/hde-epic023/acceptance_map_viability.log`, `audit/qa/hde-epic023/qa_step_logs_manifest.json`, and `audit/qa/hde-epic023/evidence_index_snapshot.json` cover map/matrix viability under closed rails. Doc-delta + consult evidence lives at `audit/docdeltas/hde-epic023_doc_deltas.md` and `audit/qa/hde-epic023/00_meta/pf23_consult.md`.
- Close-pack (EPIC023): `audit/EPIC-023_MANIFEST.json`, `audit/EPIC-023_close_report.md` (includes “QA Rails — Open/Close (Final PR)” and tracked issues TI-023-01/02/03).
- Acceptance alignment test (closed rails): `python -m pytest tests/qa/test_epic023_acceptance_alignment.py` enforces token registry membership, acceptance map ↔ token matrix lockstep, Index + Mirror coverage with proof anchors, and rejects `.path_proof.txt` as primary evidence. The canonical token set is `QA_ACCEPTANCE_MAP_VIABILITY_OK`, `EVIDENCE_INDEX_MIRROR_OK`, `EVIDENCE_PATHS_VALIDATED_OK`, `SANITY_PIPELINE_OK`, `DETERMINISM_ENV_PINS_OK`, `JSON_CANONICAL_CHECK_OK`, `DOC_DELTA_PRESENT_OK`, `TWO_RUN_IDENTITY_OK`.
...
- Orientation and sanity checks: `python tools/evidence/update_evidence_index.py` (write) then `python tools/evidence/orientation_demo.py` (write) followed by their `--check` variants; `python tools/evidence/run_sanity_pipeline.py` (closed rails); `python ci/checks/check_mirror_schema.sh` enforces mirror schema/self-record/path-proof discipline (Python entrypoint despite `.sh` suffix).
...
- EPIC022 acceptance scaffolding: `docs/acceptance_map_epic022.json` and `audit/qa/hde-epic022/token_evidence_matrix.md` bind tokens to evidence and tests; close-pack artifacts live in `audit/EPIC-022_MANIFEST.json` and `audit/EPIC-022_close_report.md`. EPIC023 extends this with the canonical 8-token acceptance set, map/matrix/viability bindings under `docs/acceptance_map_epic023.json` and `audit/qa/hde-epic023/`, and governed close-pack artifacts in `audit/EPIC-023_MANIFEST.json` and `audit/EPIC-023_close_report.md`.
...
- Acceptance alignment (EPIC023): `tests/qa/test_epic023_acceptance_alignment.py` enforces token registry membership, map ↔ matrix parity, governed evidence paths, and Index/Mirror proof anchors for implemented tokens.
