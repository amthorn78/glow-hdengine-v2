...
- What EPIC025 tightens:
  - `/api/compat/v1` contract: GET is probe-only (rejects request bodies) and returns `{ok:true,schema:"v1"}`; POST is the compat computation surface with request-shape rules (reject mixed id+payload patterns) and strict id validation against `UID_RE` (invalid IDs return an `invalid_json` error envelope with a 400 status). In prod, `APP_ENV=prod` returns 404 with `ERR_NOT_FOUND` (implementation in `engine/http/compat_handler.py`; id regex in `engine/compat/ordering.py`). Endpoint catalog entry is POST-only, `internal_admin`, `a7_eligible: false`, and `env_gate` is non-empty (`docs/ENDPOINTS_CATALOG.json` + `.sha256`).
  - CLI `showcompat` is coupled to the canonical emitter for deterministic serialization (entrypoint in `engine/cli/main.py`, emitter in `engine/presenter/emitter.py`). Stdout must end with exactly one LF; CRLF is rejected with explicit error codes (`STDOUT_MISSING_LF`, `STDOUT_CRLF`). Optional `--dump-reader` and `--dump-admin-dir` sidecars write Reader bytes/admin proofs without affecting stdout determinism.
  ...
  - EPIC025 close-pack artifacts are generated by `tools/qa/generate_epic025_close_pack.py` and recorded at `audit/EPIC-025_MANIFEST.json`, `audit/EPIC-025_close_report.md`, and doc deltas at `audit/docdeltas/hde-epic025_doc_deltas.md`.
...
- EPIC025 close pack: `python tools/qa/generate_epic025_close_pack.py` writes `audit/EPIC-025_MANIFEST.json`, `audit/EPIC-025_close_report.md`, and `audit/docdeltas/hde-epic025_doc_deltas.md` with QA step manifest and path proofs under `audit/qa/hde-epic025/`.
...
- `showcompat` accepts `--pair-file`, `--a-file/--b-file` (aliases `--a/--b`), stdin, or birth arguments; success prints LF-terminated canonical compat payload to stdout (includes numeric scores/weights captured in EPIC022 D2). Stdout must end with exactly one LF; CRLF is rejected with error codes `STDOUT_MISSING_LF` and `STDOUT_CRLF`. Sidecars: `--dump-reader` (Reader v1 bytes via the presenter path), `--dump-admin-dir` (admin proofs under the provided directory).
