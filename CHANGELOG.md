...
### Added
- Documented HDE-EPIC038 as a docs-visible Distillation reliability slice covering direct-only PostgreSQL transport, manifest-derived release identity, updater-owned evidence graph discipline, configured-v2 mapped-cache boundaries, and its historical nineteen-stage release-sanity chain. Under PF10 §2.3, current exact-source admission is the fifteen-stage generic chain; frozen historical-bridge and OPS packets are no longer current release gates.
...
### Changed / Fixed
- Refreshed top-level release-attestation wording from PR-A/stage-14-stop posture to the current exact-source external attestation posture: the fifteen-stage generic gate emits PF12's `PR06R_B_FINAL_PASS` v1 compatibility value without claiming HDE-EPIC038 completion, QA PASS, acceptance, PF09 movement, OPS, deployment, or closeout.
...
### Added
- Documented explicit conjunction compat `identity_hash` capture and evidence-index coverage (`artifacts/compat/identity_hash.txt` in `docs/evidence/INDEX.json` + `artifacts/evidence_index.jsonl`).
- Documented canonical EPIC027 acceptance-ledger and close-pack homes: `docs/acceptance_map_epic027.json`, `audit/qa/hde-epic027/token_evidence_matrix.md`, `audit/qa/hde-epic027/acceptance_map_viability.log`, `audit/EPIC-027_MANIFEST.json`, and `audit/EPIC-027_close_report.md`.
- Documented CLI installability/help/version evidence generated by `tools/cli/generate_cli_conformance_artifacts.py` under `artifacts/cli/install/` and `artifacts/cli/help/` (including console entrypoint proof in `artifacts/cli/install/entrypoints.txt`).

### Changed / Fixed
- Refreshed README/AGENTS/docs references from EPIC026-focused QA ledger wording to EPIC027 current surfaces, including conjunction writer readback evidence (`artifacts/writer/conjunction_write_readback.log`, `artifacts/writer/conjunction_writer_summary.json`).
- Clarified writer conjunction evidence posture: open rails for provider acquisition are caller-provided and explicit (`SAFE_MODE=0` + `ALLOW_NETWORK=1`) rather than silently forced open by tooling.
