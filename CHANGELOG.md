...
Unreleased — EPIC-022: Separation Pass 2 docs and evidence tightening

### Added
- D1 refusal/error-envelope parity enumerated for `db_unavailable` and `vendor_attempt_closed_rails` CLI/HTTP pairs under closed rails, with validators called out.
- D2 showcompat deterministic stdout capture documented against governed artifacts (`artifacts/cli/showcompat/stdout.json`, `.sha256`, `args.json`) generated by `tools/cli/generate_showcompat_artifacts.py`, with LF/stream discipline tests and acceptance bindings in the EPIC022 token matrix.
- D3 `/internal/version` evidence bundle (body, headers, conditional headers, two-run identity, release-id recompute) documented as present with path proofs and the governed coupling log.
- Evidence toolchain and rails posture tightened: closed-rails pins reiterated; refresh order now covers `update_evidence_index.py` → `orientation_demo.py` → `ci/checks/check_mirror_schema.sh` (Python entrypoint) alongside the sanity pipeline and registry report.
- Mirror/index discipline clarified for governed artifacts, including the machine-mirror `mirror_body_sha256` self-record, path-proof expectations, and INDEX.sha256 sentinel posture.
- Close-pack readiness asserted (EPIC022 manifest, close report, acceptance map, token/evidence matrix, sanity log) with no placeholders.

### Changed / Fixed
- CLI docs now pin exit semantics to tested behavior (success=0; usage/validation/IO errors=64; vendor/engine failures=1) while noting PF05 is the canonical taxonomy home; stream discipline and Reader sidecars are reiterated.
- Error-envelope and refusal parity docs refreshed for the closed-rails posture (EPIC022 D1), reinforcing env-pin gating, deterministic parity fixtures, and stream discipline expectations for CLI/Reader parity.
- README/AGENTS/docs updated to reflect EPIC022 acceptance scaffolding (manifest + close report present), deterministic showcompat captures, `/internal/version` D3 evidence with the PF20/PF10 provenance-note mismatch called out, and evidence hygiene (Index/Mirror + path proofs + mirror schema check) without placeholders.
