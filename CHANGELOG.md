...
Unreleased — HDE-EPIC024: Repo Docs Sweep (README/CHANGELOG/AGENTS/docs/)

### Added
- Documented EPIC024 QA harness outputs and acceptance map bindings: `docs/acceptance_map_epic024.json`, `audit/qa/hde-epic024/token_evidence_matrix.md`, `audit/qa/hde-epic024/acceptance_map_viability.log`, `audit/qa/hde-epic024/qa_step_logs_manifest.json`, and per-check logs under `audit/qa/hde-epic024/checks/<check_id>/primary.log` plus the EPIC024 doc-delta artifacts (`audit/docdeltas/hde-epic024_doc_deltas.md`, `audit/qa/hde-epic024/00_meta/doc_deltas.md`).
- Canonical JSON gate family now documented as `audit/gates/json_gate/canonical/` (`json_gate_check_log.ndjson`, `json_gate_compare_log.ndjson`, `json_gate_structured_record.json` + `.path_proof.txt`) produced by `python tools/evidence/run_canonical_json_gate.py`, with the legacy catalog check report preserved at `audit/gates/canonical_json/json_canonical_check.log`.
- Evidence index snapshot gate documented as `audit/gates/evidence_index_snapshot/evidence_index_snapshot.json` (+ `.path_proof.txt`) produced by `python tools/evidence/generate_evidence_index_snapshot.py`, with pytest validation coverage in `tests/evidence/test_evidence_index_snapshot.py`.
- Arrays-as-sets report documented as `artifacts/canonical/arrays_as_sets_report.log` generated by `tools/evidence/generate_arrays_as_sets_report.py`, with proof coverage in `tests/compare/test_arrays_as_sets.py`.

### Changed / Fixed
- README/AGENTS/docs refreshed to align EPIC024 evidence posture (json_gate canonical family, evidence index snapshot, arrays-as-sets report) and EPIC024 close-pack artifacts (`audit/EPIC-024_MANIFEST.json`, `audit/EPIC-024_close_report.md`).
- Evidence discipline language updated to treat `.path_proof.txt` siblings as governed outputs and to avoid treating legacy canonical JSON locations as the canonical gate family.
