...
### Added
- Showcompat deterministic stdout capture (EPIC022 D2) documented and tied to governed artifacts (`artifacts/cli/showcompat/stdout.json`, `.sha256`, `args.json`) generated by `tools/cli/generate_showcompat_artifacts.py`, with acceptance bindings in the EPIC022 token matrix.

### Changed / Fixed
- CLI/README docs now reflect tested behavior: showcompat stdout is the canonical compat payload (includes numeric scores/weights), Reader v1 bytes are emitted via `--dump-reader`, and typed failures return exit code 1 (usage errors remain 64).
...
- README/AGENTS/docs updated to reflect EPIC022 acceptance scaffolding, deterministic showcompat captures, and evidence hygiene (Index/Mirror + path proofs) without placeholders.
