...
### Added
- D1 refusal/error-envelope parity called out as closed-rails-first with env-pin gating, matching the stored parity captures.
- D2 showcompat deterministic stdout capture documented against governed artifacts (`artifacts/cli/showcompat/stdout.json`, `.sha256`, `args.json`) generated by `tools/cli/generate_showcompat_artifacts.py`, with acceptance bindings in the EPIC022 token matrix.
- D3 `/internal/version` evidence bundle (body, headers, conditional headers, two-run identity, release-id recompute) documented as present with path proofs.
- Mirror/index discipline clarified for governed artifacts, including the machine-mirror `mirror_body_sha256` self-record and orientation-demo order.

### Changed / Fixed
- CLI docs now pin exit semantics to tested behavior: success=0; usage/validation/IO errors=64; vendor/engine failures=1; Reader bytes flow through the `--dump-reader` sidecar.
- Error-envelope and refusal parity docs refreshed for the closed-rails posture (EPIC022 D1), reinforcing env-pin gating and stream discipline expectations for CLI/Reader parity.
- README/AGENTS/docs updated to reflect EPIC022 acceptance scaffolding (manifest + close report present), deterministic showcompat captures, `/internal/version` D3 evidence, and evidence hygiene (Index/Mirror + path proofs) without placeholders.
